...
### 2026-09-24: U+2027 구분자 변환 정정

위 SHA-256과 같은 2017 원본의 `천만02‧천만01/천만03`,
`셋째02‧셋째01/셋째03`, `첫째02‧첫째01/첫째03`, `소극적01‧소극적02`
네 행에서 U+2027이 누락된 변환 규칙을 수정했다. 표제어·품사를 위치대로
분리하고 동형어 번호·등급·길잡이말을 보존했다. 원본 파일과 앱 학습 데이터,
검토 원장·초안·별칭·문법 목록은 변경하지 않았다.

파생 어휘 행은 11,097→11,101이지만, 잘못 합쳐진 문자열이 제거되어 고유
표제어 분모는 A1 714→713, A2 1070→1068이다. 해당 레벨 보유 수 596/204는
그대로다. 이 변화는 콘텐츠 추가나 사람 검수 승인이 아니다.

Batch 33/34의 당시 manifest와 검토 패킷은 역사 기록으로 보존한다. 현재 원본
변환으로 재계산한 A2 비접사 raw 고유 문자열은 1083(당시1085), Batch33까지의
live+draft 제외 잔여는 597(당시599), Batch34까지는533(당시535)이다. 이는
live-only 정규화 결손656과 다른 지표이며 초안 승격을 뜻하지 않는다.
