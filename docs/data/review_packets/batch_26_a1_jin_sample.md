...
> **인물별 말투 마커 (2026-09-15, 코디네이터 추가 지시):** 이름이 등장하는 11개 행에 인물별 고유 어투를 반영했다(문장당 마커 1개 이하, 1급 문법 범위):

| 표제어 | ID | 인물 | 마커 | 예문 KO |
|---|---|---|---|---|
| 게임 | `vocab_a1_0523` | 마야 | 고에너지 감탄 '와!' + 이름 부르기 | 와, 마야 씨! 이 게임 할까요? |
| 미국 | `vocab_a1_0528` | 마야 | 고에너지 강조 '진짜' | 마야 씨는 진짜 미국에서 왔어요. |
| 사월 | `vocab_a1_0532` | 크리스티안 | 확인형 '…맞아요?' | 사월에 크리스티안이 한국에 와요, 맞아요? |
| 오월 | `vocab_a1_0533` | 크리스티안 | 머뭇거림 '음…' | 음... 오월에 크리스티안 생일이에요. |
| 일본 | `vocab_a1_0541` | 다니엘 | 영상·시각 은유(찍다) | 다니엘 씨는 일본을 찍고 싶어요. |
| 테니스 | `vocab_a1_0556` | 다니엘 | 쾌활한 안심시키기(-지만 괜찮아요) | 다니엘 씨는 테니스를 못하지만 괜찮아요. |
| 등산 | `vocab_a1_0559` | 안드레아 | 격식체 -습니다(업무·formal 어투) | 안드레아 씨는 주말에 등산을 합니다. |
| 산책 | `vocab_a1_0560` | 현아 | 차분한 되묻기 '왜요?' 계열 | 현아 씨는 왜 산책을 좋아해요? |
| 춤 | `vocab_a1_0561` | 레나 | 밝은 제안형 '…할까요?' | 레나 씨, 함께 춤을 출까요? |
| 노래 | `vocab_a1_0562` | 마야 | 고에너지 감탄 | 마야 씨, 이 노래 진짜 좋아요! |
| 떡볶이 | `vocab_a1_0566` | 수진 | 짧고 담백한 사실 진술(꾸밈 없음) | 어제 수진 씨하고 떡볶이 먹었어요. |
...
- 예문 KO: 마야 씨는 진짜 미국에서 왔어요.
- 예문 DE: Maya kommt wirklich aus den USA.
- 예문 EN: Maya really is from the USA.
- Cloze `cloze_a1_0462`: 마야 씨는 진짜 ＿＿＿에서 왔어요. → 정답 `미국` · 배분어 ['농구', '테니스', '쓰다']
- Satz `satz_a1_0443`: 목표 `마야 씨는 진짜 미국에서 왔어요.` · 배분 타일 ['안경', '모자']
- Jin 판정: 
...
- Cloze `cloze_a1_0495`: 레나 씨, 함께 ＿＿＿을 출까요? → 정답 `춤` · 배분어 ['생일', '날짜', '참']
...
- Cloze `cloze_a1_0516`: ＿＿＿에 올라갈까요? → 정답 `남산` · 배분어 ['시월', '십일월', '팔다']
- Satz `satz_a1_0497`: 목표 `남산에 올라갈까요?` · 배분 타일 ['케이크', '귤']
- Jin 판정: 
...
| **표본** | `vocab_a1_0528` | 미국 | miguk | `a1_particles_in_use_1` | 12 | die USA, Amerika | the USA, America | 마야 씨는 진짜 미국에서 왔어요. |
...
|  | `vocab_a1_0532` | 사월 | sawol | `a1_months_1` | 4 | April | April | 사월에 크리스티안이 한국에 와요, 맞아요? |
|  | `vocab_a1_0533` | 오월 | owol | `a1_months_1` | 5 | Mai | May | 음... 오월에 크리스티안 생일이에요. |
...
|  | `vocab_a1_0541` | 일본 | ilbon | `a1_countries_1` | 1 | Japan | Japan | 다니엘 씨는 일본을 찍고 싶어요. |
...
|  | `vocab_a1_0556` | 테니스 | teniseu | `a1_hobbies_1` | 4 | Tennis | tennis | 다니엘 씨는 테니스를 못하지만 괜찮아요. |
...
|  | `vocab_a1_0559` | 등산 | deungsan | `a1_hobbies_1` | 7 | Bergwandern | hiking | 안드레아 씨는 주말에 등산을 합니다. |
|  | `vocab_a1_0560` | 산책 | sanchaek | `a1_hobbies_1` | 8 | Spaziergang | walk, stroll | 현아 씨는 왜 산책을 좋아해요? |
...
- [x] 자동 회귀: `tools/content_factory/test_batch_26_draft.py` 36개 테스트 전부 통과 — 행 수 60~68, 라이브/Batch25 중복 없음, `pack_id` 라이브 또는 신규 선언, 예문 ≤8어절, 정답이 예문에 포함, 금지 문형 0건, cloze 배분어 답 비유출·중복 없음·동일 POS(명사) 2/3 이상·문장 내 미등장·단어당 재사용 4회 이하(`test_no_distractor_word_reused_more_than_4_times`), 인물명 캐논 목록 내, 프레임 3회 초과 없음.
...
| `cloze_a1_0462` | 마야 씨는 진짜 농구에서 왔어요. | ✗(의미 불성립) |
| `cloze_a1_0462` | 마야 씨는 진짜 테니스에서 왔어요. | ✗(의미 불성립) |
| `cloze_a1_0462` | 마야 씨는 진짜 쓰다에서 왔어요. | ✗(비문) |
| `cloze_a1_0463` | 제 생일은 축구이에요. | ✗(비문) |
| `cloze_a1_0463` | 제 생일은 야구이에요. | ✗(비문) |
...
| `cloze_a1_0466` | 등산에 크리스티안이 한국에 와요, 맞아요? | ✗(의미 불성립) |
| `cloze_a1_0466` | 산책에 크리스티안이 한국에 와요, 맞아요? | ✗(의미 불성립) |
| `cloze_a1_0466` | 모르다에 크리스티안이 한국에 와요, 맞아요? | ✗(비문) |
| `cloze_a1_0467` | 음... 춤에 크리스티안 생일이에요. | ✗(의미 불성립) |
| `cloze_a1_0467` | 음... 노래에 크리스티안 생일이에요. | ✗(의미 불성립) |
| `cloze_a1_0467` | 음... 좀에 크리스티안 생일이에요. | ✗(비문) |
...
| `cloze_a1_0475` | 다니엘 씨는 십일월을 찍고 싶어요. | ✗(의미 불성립) |
| `cloze_a1_0475` | 다니엘 씨는 십이월을 찍고 싶어요. | ✗(의미 불성립) |
| `cloze_a1_0475` | 다니엘 씨는 아주을 찍고 싶어요. | ✗(비문) |
...
| `cloze_a1_0485` | 책상는 어디에 있어요? | ✗(비문) |
| `cloze_a1_0485` | 안경는 어디에 있어요? | ✗(비문) |
| `cloze_a1_0485` | 가끔는 어디에 있어요? | ✗(비문) |
| `cloze_a1_0486` | 모자 생활이 어때요? | ✗(의미 불성립) |
| `cloze_a1_0486` | 신발 생활이 어때요? | ✗(의미 불성립) |
| `cloze_a1_0486` | 빌리다 생활이 어때요? | ✗(비문) |
| `cloze_a1_0487` | 저는 핸드폰를 잘해요. | ✗(비문) |
| `cloze_a1_0487` | 저는 책를 잘해요. | ✗(비문) |
| `cloze_a1_0487` | 저는 다시를 잘해요. | ✗(비문) |
...
| `cloze_a1_0490` | 다니엘 씨는 할아버지를 못하지만 괜찮아요. | ✗(의미 불성립) |
| `cloze_a1_0490` | 다니엘 씨는 부모님를 못하지만 괜찮아요. | ✗(비문) |
| `cloze_a1_0490` | 다니엘 씨는 오다를 못하지만 괜찮아요. | ✗(비문) |
...
| `cloze_a1_0493` | 안드레아 씨는 주말에 월요일을 합니다. | ✗(의미 불성립) |
| `cloze_a1_0493` | 안드레아 씨는 주말에 밤을 합니다. | ✗(의미 불성립) |
| `cloze_a1_0493` | 안드레아 씨는 주말에 좀을 합니다. | ✗(비문) |
| `cloze_a1_0494` | 현아 씨는 왜 기분을 좋아해요? | ✗(의미 불성립) |
| `cloze_a1_0494` | 현아 씨는 왜 값을 좋아해요? | ✗(의미 불성립) |
| `cloze_a1_0494` | 현아 씨는 왜 쓰다을 좋아해요? | ✗(비문) |
| `cloze_a1_0495` | 레나 씨, 함께 생일을 출까요? | ✗(의미 불성립) |
| `cloze_a1_0495` | 레나 씨, 함께 날짜을 출까요? | ✗(비문) |
| `cloze_a1_0495` | 레나 씨, 함께 참을 출까요? | ✗(비문) |
...
| `cloze_a1_0502` | 필요를 좋아하세요? | ✗(의미 불성립) |
| `cloze_a1_0502` | 기분를 좋아하세요? | ✗(비문) |
| `cloze_a1_0502` | 빌리다를 좋아하세요? | ✗(비문) |
| `cloze_a1_0503` | 칠월이 몸에 좋아요. | ✗(의미 불성립) |
| `cloze_a1_0503` | 팔월이 몸에 좋아요. | ✗(의미 불성립) |
| `cloze_a1_0503` | 주로이 몸에 좋아요. | ✗(비문) |
| `cloze_a1_0504` | 생일에 구월를 먹어요. | ✗(비문) |
| `cloze_a1_0504` | 생일에 시월를 먹어요. | ✗(비문) |
| `cloze_a1_0504` | 생일에 다니다를 먹어요. | ✗(비문) |
| `cloze_a1_0505` | 저는 십일월을 자주 먹어요. | ✗(의미 불성립) |
| `cloze_a1_0505` | 저는 십이월을 자주 먹어요. | ✗(의미 불성립) |
| `cloze_a1_0505` | 저는 꼭을 자주 먹어요. | ✗(비문) |
| `cloze_a1_0506` | 이 일본가 진짜 맛있어요. | ✗(비문) |
| `cloze_a1_0506` | 이 중국가 진짜 맛있어요. | ✗(비문) |
| `cloze_a1_0506` | 이 오다가 진짜 맛있어요. | ✗(비문) |
| `cloze_a1_0507` | 기분 좀 주세요. | ✗(의미 불성립) |
| `cloze_a1_0507` | 일월 좀 주세요. | ✗(의미 불성립) |
| `cloze_a1_0507` | 함께 좀 주세요. | ✗(비문) |
| `cloze_a1_0508` | 할머니가 값을 좋아하세요. | ✗(의미 불성립) |
| `cloze_a1_0508` | 할머니가 필요을 좋아하세요. | ✗(비문) |
| `cloze_a1_0508` | 할머니가 읽다을 좋아하세요. | ✗(비문) |
| `cloze_a1_0509` | 이월 있어요? | ✗(의미 불성립) |
| `cloze_a1_0509` | 삼월 있어요? | ✗(의미 불성립) |
| `cloze_a1_0509` | 빨리 있어요? | ✗(비문) |
| `cloze_a1_0510` | 사월 하나 더 주세요. | ✗(의미 불성립) |
| `cloze_a1_0510` | 오월 하나 더 주세요. | ✗(의미 불성립) |
| `cloze_a1_0510` | 쓰다 하나 더 주세요. | ✗(비문) |
| `cloze_a1_0511` | 저는 사이다에 살아요. | ✗(의미 불성립) |
| `cloze_a1_0511` | 저는 콜라에 살아요. | ✗(의미 불성립) |
| `cloze_a1_0511` | 저는 가끔에 살아요. | ✗(비문) |
| `cloze_a1_0512` | 이번 주말에 책상에 갈까요? | ✗(의미 불성립) |
| `cloze_a1_0512` | 이번 주말에 안경에 갈까요? | ✗(의미 불성립) |
| `cloze_a1_0512` | 이번 주말에 입다에 갈까요? | ✗(비문) |
| `cloze_a1_0513` | 유월공항에서 만나요. | ✗(의미 불성립) |
| `cloze_a1_0513` | 칠월공항에서 만나요. | ✗(의미 불성립) |
| `cloze_a1_0513` | 다시공항에서 만나요. | ✗(비문) |
| `cloze_a1_0514` | 모자에 가고 싶어요. | ✗(의미 불성립) |
| `cloze_a1_0514` | 신발에 가고 싶어요. | ✗(의미 불성립) |
| `cloze_a1_0514` | 모르다에 가고 싶어요. | ✗(비문) |
| `cloze_a1_0515` | 팔월 구경 갈까요? | ✗(의미 불성립) |
| `cloze_a1_0515` | 구월 구경 갈까요? | ✗(의미 불성립) |
| `cloze_a1_0515` | 바로 구경 갈까요? | ✗(비문) |
| `cloze_a1_0516` | 시월에 올라갈까요? | ✗(의미 불성립) |
| `cloze_a1_0516` | 십일월에 올라갈까요? | ✗(의미 불성립) |
| `cloze_a1_0516` | 팔다에 올라갈까요? | ✗(비문) |
| `cloze_a1_0517` | 핸드폰에서 옷을 샀어요. | ✗(의미 불성립) |
| `cloze_a1_0517` | 책에서 옷을 샀어요. | ✗(의미 불성립) |
| `cloze_a1_0517` | 이따가에서 옷을 샀어요. | ✗(비문) |
| `cloze_a1_0518` | 물건에서 책을 골랐어요. | ✗(의미 불성립) |
| `cloze_a1_0518` | 케이크에서 책을 골랐어요. | ✗(의미 불성립) |
| `cloze_a1_0518` | 빌리다에서 책을 골랐어요. | ✗(비문) |
| `cloze_a1_0519` | 이 근처에 십이월이 있어요? | ✗(의미 불성립) |
| `cloze_a1_0519` | 이 근처에 책상이 있어요? | ✗(의미 불성립) |
| `cloze_a1_0519` | 이 근처에 좀이 있어요? | ✗(비문) |
| `cloze_a1_0520` | 저녁에 귤에 갈까요? | ✗(의미 불성립) |
| `cloze_a1_0520` | 저녁에 바나나에 갈까요? | ✗(의미 불성립) |
| `cloze_a1_0520` | 저녁에 다니다에 갈까요? | ✗(비문) |
| `cloze_a1_0521` | 이 근처에 안경이 있어요? | ✗(의미 불성립) |
| `cloze_a1_0521` | 이 근처에 모자이 있어요? | ✗(비문) |
| `cloze_a1_0521` | 이 근처에 참이 있어요? | ✗(비문) |
| `cloze_a1_0522` | 저는 우유에 자주 가요. | ✗(의미 불성립) |
| `cloze_a1_0522` | 저는 할머니에 자주 가요. | ✗(의미 불성립) |
| `cloze_a1_0522` | 저는 보다에 자주 가요. | ✗(비문) |
