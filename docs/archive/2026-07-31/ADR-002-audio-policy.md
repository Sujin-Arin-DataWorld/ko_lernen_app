> ⚠️ 구초안 (2026-08-01) — 최종본은 [docs/ADR-002-audio-policy.md](../../ADR-002-audio-policy.md). 최종본에서 정정 2건("볼륨을 코드로 못 줄인다"→틀림 / 게인표 −40dB 실측 교체) 반영됨. 기록 보존용.
...
| 에셋 | 실측 mean | 필요 게인 | linear |
|---|---|---|---|
| `hanok_construction` | −19.6 | −16.4 dB | **0.15** |
| `kkeunmari_hero` | −27.1 | −8.9 dB | **0.36** |
| `listening_hero` | −33.3 | −2.7 dB | **0.73** |
| `hanok_jongga` | −35.2 | −0.8 dB | **0.91** |
| `welcome-hero` | −35.5 | −0.5 dB | **0.94** |
| `study_scholar` | −40.0 | (+4.0) | 1.00 |
| `study_classroom` | −45.8 | (+9.8) | 1.00 |
| `porch` | −48.6 | (+12.6) | 1.00 |
...
2. **새로 만들 영상에 오디오를 넣을 것인가.** 캐릭터 클립 16개는 전부 무음이라
   포효·짹짹이 mp3 로 나간다. 영상에 소리를 실으면 립싱크가 맞지만
   **볼륨을 코드로 못 줄인다**(§4 게인으로만 가능). 지금처럼 mp3 분리가 제어에는 유리하다.
