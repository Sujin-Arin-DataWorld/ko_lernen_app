...
> (`packs/`·`activities/`·`listening/` 의 800×600 카드)에는 적용되지 않는다. 그 계열의
> 실측 정본은 소프트 셰이딩이 있다 — 도자기·놋쇠의 스페큘러, 면내 톤 그라데이션
> (`octagon` 옹기·`lotus` 청자·`A1Arrival` 여행가방 실측). 3번을 그대로 프롬프트에
> 넣어 2026-08-30 C1/C2 23장이 평면 벡터로 나와 전량 폐기됐다. 이 계열은
> `docs/LISTENING_CARD_RECIPE.md`(생성 정본) + `docs/assets/STYLE_LOCK.json` families
> `F-E-cards`(실측 불변값)를 따른다. 3번은 한옥/장식 faceted 계열(F-A/F-B 등)에는
> 여전히 유효하다 — 전역 폐기가 아니라 계열 스코프 정정이다.
