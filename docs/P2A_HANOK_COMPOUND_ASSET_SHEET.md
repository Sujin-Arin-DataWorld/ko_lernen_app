...
> **R1 카메라 보정 (2026-08-04):** 첫 번째 독립 3/4 건물 시안은
> 구조물별 소실점이 달라 한 장의 배치도로 읽히지 않아 사용자 피드백으로
> 반려됐다. 아래의 `elevated three-quarter` 표현은 항상 **north-up
> plan-locked oblique**, `map south = lower edge`, `single shared yaw`,
> `east-west roof ridge = horizontal`을 뜻한다. R1에서는 `site_base`와
> 같은 방향의 더 평면적인 지붕 발자국을 우선한다.
