...
> **2026-09-03 현재 런타임:** 사용자 원본 책장 복원 요청에 따라
> `ChaekgadoShelfCase`는 supplied bookcase 프레임을 런타임에서 잘라 쓰고,
> 칸 그림은 `BoxFit.contain`, 이름표는 그림 아래 별도 행으로 표시한다.
> 첫 전폭 칸을 포함해 원본 그림을 자르거나 이름표로 덮지 않는다.
> 아래 2026-09-01 가시율 수치는 이전 `cover`/이름표 오버레이 구현의
> 역사적 측정이며 현재 칸 레이아웃의 생성·배치 기준으로 사용하지 않는다.
> 카드 원본 파일의 800×600 규격과 원본 유지 계약은 그대로다.
