...
- **Version:** 1.25
- **Created:** 2026-08-21
- **Branch:** `session/uiux-bible-learning-3b-cloze-closeout-2026-08-21`
- **Base:** `origin/main@c3fc9fbe7cc5a82dc493458c3cf6cd95989d90a1`
- **State:** Phase 3B Cloze outer UI merged; clean restart boundary
- **Next action:** create a fresh worktree from latest `origin/main` and audit
  `/daily` app-owned outer hierarchy/actions only; preserve daily seed, item
  order, answer checking, first-attempt scoring, already-done state, feedback,
  completion, and one-time reward logic
...
| 3B–3D remaining learning flows | pending | next unit: create a fresh worktree from latest `origin/main` and audit `/daily` app-owned outer hierarchy/actions only; preserve daily seed, item order, answer checking, first-attempt scoring, already-done state, feedback, completion, and one-time reward logic |
