...
- **State:** Phase 4A `/stats`, `/profile`, and `/settings` are merged; the
  current Paywall head includes both first-review P2 corrections
- **Next action:** run both review axes on the current corrected commit and push
  only a zero-finding exact head for automatic PR CI
...
a close icon whose hardcoded enabled color hid its disabled visual state; the
current head corrects both and awaits the two repeat review axes.
