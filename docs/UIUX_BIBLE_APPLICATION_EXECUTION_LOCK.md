...
Start with only `AGENTS.md` and this file. Preserve progress/stat aggregation,
chart data and semantics, storage writes, routes, and approved assets. No
deployment, store upload, production mutation, asset generation, new token, or
navigation/state rewrite is authorized. Account, purchase, notification, and
platform-owned settings remain separate high-risk units.
