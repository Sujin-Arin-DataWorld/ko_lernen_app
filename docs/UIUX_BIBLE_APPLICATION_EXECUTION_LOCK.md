...
- **Version:** 1.32
- **Created:** 2026-08-21
- **Branch:** `session/uiux-bible-learning-3b-satz-2026-08-21`
- **Base:** `origin/main@aabb894c1e065d08fee640ccedaa78cf8f16511b`
- **State:** Phase 3B Satz outer UI is local green; delivery gates pending
- **Next action:** commit and push the four task-owned files, open the app PR,
  require automatic CI green for its exact current head, merge, cancel only the
  exact post-merge main-push run, and prove its Signed AAB/Play job executed
  zero steps before recording the clean Phase 3C restart point
...
| 3B Satz outer UI | local green | Branch `session/uiux-bible-learning-3b-satz-2026-08-21` from `origin/main@aabb894c1e065d08fee640ccedaa78cf8f16511b`; `/satz_arcade` carries round progress plus score in the shared Study eyebrow, exposes a localized 48dp close action and a visible localized level-filter name, gives every level chip a 48dp complete-text contract, and routes instructions and inline diagnostics through existing Sori type roles; the existing `SoriWordTile` 48dp contract is restored at 320dp by retaining the 3/4-column grid and applying a minimum row extent; DE/EN 320x640 at 200%, 360x400, 390x844, 720x1024, and 1280x900 covered; analyze clean; 106 focused Satz/data/order/diagnosis/mission/evidence/feedback tests and 924 shared responsive/short-height/accessibility/smoke/typography/localization tests green; raw screen TextStyle 287→285; course-scoped catalog filtering, initial mission item, tile identity/order and tap gestures, punctuation/order/particle/count diagnosis, retry/reveal timing, SRS, exact activity evidence, score/XP/best, feedback completion, TTS, routes, and approved assets unchanged; app PR, exact-head CI, merge, and post-merge Play zero-step proof pending |
| 3C–3D remaining learning flows | pending | after the Satz merge and clean closeout, create a fresh worktree from latest `origin/main` and audit the smallest `/path` or `/course` evidence boundary before implementation |
