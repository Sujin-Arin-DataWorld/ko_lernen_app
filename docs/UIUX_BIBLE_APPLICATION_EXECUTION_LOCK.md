...
- **Version:** 1.43
- **Created:** 2026-08-21
- **Branch:** `session/uiux-bible-learning-3d-word-web-closeout-2026-08-21`
- **Base:** `origin/main@29e08680129fcd2ee9377e8859f8627246605a5d`
- **State:** Phase 3D `/word_web` outer UI complete and merged; clean
  `/sarangbang` restart point recorded
- **Next action:** create a fresh worktree from latest `origin/main` and audit
  `/sarangbang` outer UI and state matrix only; preserve custom-room
  composition, recommendations/course evidence, room/canvas boundaries,
  routes and approved assets

## Current State Summary

`/word_web` is merged on `main@29e08680129fcd2ee9377e8859f8627246605a5d`
...
unit is `/sarangbang` outer UI only.

## Important Context

Start with only `AGENTS.md` and this file. Preserve custom-room composition,
recommendations/course evidence, room/canvas boundaries, routes, and approved
assets. No deployment, store upload, production mutation, asset generation,
new token, or navigation/state rewrite is authorized. `/sarangbang/furnish`
remains a later, separate embedded-owner unit.

## Immediate Next Steps

1. Fetch latest `origin/main`, verify it contains this closeout, and create a
   fresh Windows-native `/sarangbang` worktree and task branch.
2. Measure `SarangbangStudyScreen`, its outer states, recommendation/course
   evidence flow, route tests, and room/canvas ownership before deciding whether
   code changes are needed.
3. If a presentation gap exists, implement only one small `/sarangbang`
   outer-UI unit with DE/EN and the required viewport matrix; otherwise record
   no-code verification. Do not include `/sarangbang/furnish`.
