...
>
> **개정 v1.2 (2026-08-03)**: 독립 검증 에이전트 지적 15건 반영(치명 2: 코스 미션 시스템 누락·Q7 구식) · Jin 결정 8건 확정(§11) · 캐릭터 클립 3종(tiger_walking_front/roar/thinking) 교체를 Jin이 세션에 위임(§5.2). 브랜치 `feat/design-refresh-2026-08` 생성·체크아웃 완료.
...
| # | 에셋 | 사용처 | 위반 내용 | 교체 방침 |
|---|---|---|---|---|
| 5 | `assets/video/character/tiger_walking_front.mp4` | 경로 "지금" 노드 · 게임 대기 · 프로필 포즈 풀 | 정면 걷기 렌더가 캐논 대비 색 옅음·얼굴 비례 상이, 저퀄 (Jin: "퀄리티 떨어지고 어색") | **캐논 엎드림 자세 + 부드러운 상하 바운스 루프**로 재생성 — tiger_idle 직접 기반이라 정합 최상 |
...
| **R6 에셋** | — | **온보딩 4건 = Jin 제작**(라이트 배경 + Q4 부적 모티프) · **클립 3종(tiger_walking_front/roar/thinking) = 세션 생성 완료분 실기기 검수** · Joy 격차 목록 확정 | BIBLE 프롬프트 준거 + 규격 계약(§5.4) + 무문자 원칙 + 전 프레임 스캔 |
...
**에셋·캐릭터 (R6)**
- [ ] 신규 에셋: 온보딩 4건(Jin) + 클립 3종(tiger_walking_front/roar/thinking — 세션 생성) BIBLE 준거 + 전 프레임 픽셀 스캔 리포트
