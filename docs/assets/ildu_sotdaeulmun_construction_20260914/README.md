...
- 순서·한국어 문장·크기·SHA-256: `docs/assets/ildu_sotdaeulmun_construction_20260914/construction_catalog.json`
...
이번 등록은 승인된 공정 아트 정본을 확정한다. 아직 앱 화면에서 사용하지 않는 PNG와 카탈로그는 실행 번들에서 제외한다. 실제 화면에 연결할 때 필요한 에셋만 번들에 등록한다. 전체 지도 합성용 투명 레이어, 부재별 조립 애니메이션, 한국어 학습 완료에 따른 진행도 연결은 별도 구현 범위다. 완성형의 8방향 회전 자료는 이 12단계와 독립적으로 관리한다.

숨은 구조는 깊이를 고려한 교육용 표현이며 실측 복원 도면이 아니다. 각 단계의 미세한 위치·재질 차이가 있을 수 있으므로 픽셀 정합 레이어로 취급하지 않는다.

## 검증

`flutter test test/ildu_sotdaeulmun_construction_art_test.dart`는 정본 폴더에서 12장 전부를 읽어 순서, SHA-256, PNG 크기, 마지막 정본 해시와 STYLE_LOCK 등록 일치를 검사한다.
