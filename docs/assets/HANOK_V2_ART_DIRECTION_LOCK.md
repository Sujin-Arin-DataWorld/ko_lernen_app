...
- Rendering: **Hangul Sori heritage 2.5D game-art system** (full statement: `STYLE_LOCK.json → F-D-ildoo.styleStatement`, Jin 확정 2026-08-23). Two registers, never mixed:
  - **Architecture register** (this estate — buildings, gates, walls, ground): stylized hand-painted hanok sprites, elevated three-quarter oblique view, believable Korean construction, precise giwa roof rhythm, warm timber, hanji plaster, stone foundations, selective ink-like linework, matte materials, **restrained painterly facets** — never triangulated geometry.
  - **Props/vignette register** (decor objects, UI vignettes — NOT this estate): stronger angular polygonal facets, hanji grain, compact silhouettes.
  - Minhwa-inspired colour and decorative vocabulary without imitating literal minhwa painting. Premium contemporary Korean game art — **not watercolor, not anime, not Pixar, not literal low-poly 3D.**
