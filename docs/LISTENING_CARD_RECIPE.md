...
1. 검은 윤곽선이 없는가
2. 사물이 아래 1/3, 위가 비어 있는가 — 질량중심은 세로 **60~77%** (하한 쪽이 안전, 아래 2026-09-01 절 참고)
...
> **⚠️ 2026-09-01 — 2번과 6번의 겉보기 충돌.** "아래 1/3에 배치"(2번)는 `SoriIllustratedCard`
> 세이프 영역(`docs/LISTENING_CARD_ART_SPEC.md` 🔴절, 16:10 크롭) 기준으로 옳다. 하지만 같은
> 아트가 `ChaekgadoShelfCase`(듣기 선반, 1.667 칸 크롭 + 하단 이름표)에서는 정렬을 하단
> 고정(`Alignment(0, 1.0)`)으로 고친 뒤에도 이름표가 칸 아래 ~20%를 덮는다 — 그래서 6번
> "100px 축소 판독성"을 선반 문맥에서 지키려면 질량중심을 60~77% 범위의 **하한(60% 쪽)**에
> 두는 편이 이름표에 덜 먹혀 유리하다. 77% 쪽에 두면 `SoriIllustratedCard`(카드 그리드)에서는
> 문제없지만 선반 이름표 바로 위에 걸려 6번이 약해진다. 두 화면을 다 쓰는 한 60~70% 쪽을
> 목표로 잡을 것 — 상세 실효 가시창 수치는 `docs/LISTENING_CARD_ART_SPEC.md`
> "🟠 책가도 선반의 실제 세이프 영역" 절.
