...
| 구간 | 현재 관측 | 다음 행동 |
|---|---|---|
| 웹 동의→태그 | 공개 Cloudflare 사이트와 GA 웹 ID `G-6R9J2N1PCC` 일치. 새 브라우저에서 Cookiebot이 `HANGUL-SORI.COM`을 domain group `4693fd59-1476-40b9-b6f2-091898b6732e`에 허용되지 않은 도메인으로 거절. 통계 동의 버튼 비활성, GA 전송 없음 | 사용자 Cookiebot 관리자 로그인 후 기존 그룹의 도메인·구독 상태를 확인/수정. 원인을 임의로 새 속성 생성이나 강제 추적으로 우회하지 않음 |
| GA 수신 | 웹·현재 Android/iOS·legacy iOS 스트림 모두 최근 48시간 수신 없음 표시 | 동의한 실제 배포 앱의 SDK→전송→올바른 스트림 수신 확인. 웹은 동의 후 방문·페이지 이동 각 1회, 거부는 전송 0회 확인 |
