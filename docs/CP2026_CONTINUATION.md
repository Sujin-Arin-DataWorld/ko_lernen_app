...
## 2026-09-23 20:29 UTC 현재 실행 기준선

이 절이 현재 기준이다. 뒤의 날짜별 대기 기록은 당시 증거이며, 이미 끝난 일을 다시 시작하는 근거로 쓰지 않는다. 배포·기기 설치·사람 검수·운영 품질 완료는 각각 별도다.

| 구분 | 확인한 결과 | 남은 일 |
|---|---|---|
| Android 내부 테스트 | 기존 `com.sujinarin.ko_lernen_app` **2.0.9(7653)**, SHA `833e87b72cf545ccb859147221d827ea888dd807`, run `35831739573` 성공. Play 내부 release79 테스터 제공 | 실제 업데이트·책 촬영·오프라인·진행 복구 검수 |
| 기존 iOS 내부 테스트 | 기존 Hangul Sori `6798293722` / `com.sujinarin.koLernenApp` **2.0.9(253)**, SHA `28484053759ee7e4c2bd9f01633b5cb93fa86703`, TestFlight 내부 `n.1tester` 제공 | 실제 iPhone 검수. 19:50 Cloud 재진입 때 인증 실패·로그인 폼을 확인해 다음 업로드용 로그인 복구 질문 1회 전달. 기존 배포 완료와 별개 |
| 두 내부 빌드에 포함 | 첫 안정화, 사용자 제공 책 스캔 그림 카드, 일일 학습, B3 사유 처리, 커스텀 팩 게임, #398 읽기 제한, #399 팩 백업 보존 | #397·#400 이후 후속은 아직 위 빌드에 포함되지 않음. 새 앱·외부 그룹·정식 심사와 구분 |
| 현재 main | #409 squash `80b13fa3fcf4e1ac23f7092ef09e39d015f97271`, 부모 #410 `1a5a629a`. #409 head CI/PW·리뷰0 확인 후 병합, PR/main tree 동일. 20:24 관측 main PW `35914003705` success, CI `35914003748`의 Analyze & Build 진행 중 | 정확한 main CI 완료 후 내부 배포. 이전 #410 main CI/PW 성공은 이 새 SHA 성공을 대신하지 않음 |
| 다음 내부 배포 | 현재 내부7653/253 유지. 개인정보 동의 수정이 들어간 main80b13fa3의 자동 검사 사용, 새 dispatch 없음. 예상 Android lane0 코드7680 | main CI/PW 성공과 실제 SHA·빌드번호 확인 후 기존 Android/internal 및 기존 iOS/n.1tester 배포 우선. Apple 로그인 복구 응답 대기. 다른 후속 PR 전체 완료를 기다리지 않음 |
| OCR 뜻 보완 | #405 겹조사32·서술격13은 main80 정상 통합 head `61689d79`, 새9파일161검사·분석·두 축0 후 자동검사 중. #406 지시문 head `1d35cba1` CI/PW 성공. #407 불규칙 활용23개와 DE/EN 중의성 안내의 main80 통합 검사는 성공했으며, 추가로 책장·커스텀 팩 동기화 때 대안 표제어가 빠지는 문제를 재현·수정해 검증 중 | 순서대로 최신 main 정합·PR head 검사·리뷰·병합·exact-main 검사·배포. 실제 혼합 교재 사진·사람·기기 검수 및 광범위 중의어 판정은 미완. 기존 검사 성공을 추가 수정 head의 성공으로 대신하지 않음 |
| 준비된 후속 | 운영 알림 #411, 시작 속도 도구 #412, Word Web #413의 PR CI/PW 성공. 심볼 evidence #408은 main80b13fa3 정상 통합 head `522ae709`, 계약86/도구87·skip1/두 축0 후 새 원격 검사 중 | 최신 main 정합·통합·실제 운영/기기 검증. Word Web 초안24개는 사람 및 퀴즈 승인 전 승격 차단. 심볼 gate 활성화·실제 업로드 미완 |
| 학습 범위 | 최근 소스 감사 at-level **A1 596/714, A2 204/1070**. 원본 교재·사람 승인·권리 기록 보존 | A1/A2 100%, B1 문법67, 엔진×레벨 최소5 보충 또는 숨김, B2–C2 기존 품질. 수록 범위는 사람 검수 합격률이 아님 |
| 운영·웹 | 서버 표적 배포와 로그 메트릭3개 설정 검증. 19:31 GA 웹 스트림 수신 확인, 앱 Android·기존iOS·새iOS는 최근48시간 수신 없음 | 수신 채널·알림 정책, 실제 앱 동의→SDK→수신, #390 공개 범위 및 웹 배포, Crashlytics/ANR/진행 유실·14일 적격 관측 |

Android 참조 다운로드483MB, iPhone16 다운로드500MB·설치620MB는 콘솔 추정치다. 실제 기기 측정이나 S5 기본 모듈150MB 목표 달성이 아니다. #367은 동의·Wi-Fi/모바일·오프라인 재실행·팩 삭제·원본 품질과 실제 크기 증거 전까지 HOLD한다.

현재 증거는 `C:/dev/hangulsori/_codex_artifacts/cp2026-release-stabilization-20260922/`의 `android7653-publication-receipt.json`, `ios253-publication-receipt.json`, `pr409-merge-proof.json`, `pr409-main-observation-2024.json`, `pr410-main-success.json`, `next-internal-release-plan.json`, `ocr-main410/remote-2024.json`, `instruction-main409/proof.json`, `irregular-main409/proof.json`, `symbol-main409/proof.json`, `content-coverage-main397.json`, `ga-streams-20260923-1931.txt`와 `jin-review/CHECKLIST.md`다. C01–C06 사람 검수, D01–D08 기기 검수와 14일 품질 판정은 미완이다. 서로 다른 증거를 임의 가중해 전체 완료율을 만들지 않는다.
...
| O1 OCR 뜻 | #330/#352 병합. #405는 명사 겹조사32개·서술격13개, 실제 fixture23/23토큰과 DE/EN 회귀·전체 분석·두 축 검토 통과 | #405 최신main 정합·새head검사·통합·내부배포. 불규칙·광범위 동형어·사람/실제 혼합 교재 촬영 검수 미완 |
