...
## 2026-09-23 12:12 UTC 현재 실행 기준선

이 절이 현재 기준이다. 뒤의 날짜별 배포 대기 기록은 당시의 증거이며, 이미 완료된 작업을 다시 시작하는 근거로 쓰지 않는다. 배포·기기 설치·사람 검수·운영 품질 완료는 각각 별도다.

| 구분 | 확인한 결과 | 남은 일 |
|---|---|---|
| Android 내부 테스트 | 기존 패키지 `com.sujinarin.ko_lernen_app`의 **2.0.9(7653)**, SHA `833e87b72cf545ccb859147221d827ea888dd807`. 서명 업로드 run `35831739573` 성공, Play 내부 release79가 테스터에게 제공됨 | 실제 업데이트 설치와 책 촬영·오프라인·진행 복구 검수 |
| 기존 iOS 내부 테스트 | 기존 Hangul Sori `6798293722` / `com.sujinarin.koLernenApp`의 **2.0.9(253)**, SHA `28484053759ee7e4c2bd9f01633b5cb93fa86703`. TestFlight 내부 `n.1tester` 제공 확인 | 실제 iPhone 업데이트·촬영·화면·복구 검수. 새 앱, 외부 그룹, 정식 심사와 구분 |
| 두 내부 빌드에 포함 | 첫 안정화, 사용자 제공 그림을 쓴 책 스캔 카드, 일일 콘텐츠 학습, B3 사유 처리, 커스텀 팩 게임, #398 읽기 제한, #399 팩 백업 후속 보존 | 후속 #397 및 #400–#405는 이 두 빌드의 완료 증거에 포함하지 않음 |
| 최근 검증 main | #397의 main `d93160386bea67b69ce3408ebb04afa7b1959105`, CI `35849510407` / Playwright `35849510459` 성공 | 여섯 레벨 native Phase run `35855394802`는 실제 시뮬레이터 학습 실행 중. 기기 검수와 별도 |
| 최신 병합 | #401의 head `9d547111` 검사·리뷰0 후 main `6ff600deea920fa3966eb06a446b7f60bf45df59`로 병합. PR head와 merge-main 전체 트리 동일 | 자동 main CI `35858329070` / Playwright `35858329059` 진행 중. 성공 뒤 #402 → #403 → #404 → #405 순서로 정상 main 병합·Graphify·새 head 검사를 거침 |
| OCR 뜻 보완 | #405에 명사 겹조사32개와 서술격13개 형태를 보완. 확장 회귀177개, 마지막 핵심116개는 중복 검사이며 합산하지 않음. 전체 분석과 두 축 리뷰 통과. 혼합 문장 fixture는 실제 결과로23/23토큰 검증 | 최신 main 정합·새 head 검사·병합·내부 배포, 불규칙·광범위 동형어·실제 촬영 페이지 및 사람 검수 |
| 학습 범위 | main d931 기준 정규화 표제어의 at-level 수는 **A1 596/714, A2 204/1070**. 원본 교재·사람 승인·권리 기록 보존 | A1/A2 100%, B1 문법67항목, 엔진×레벨 최소5, B2–C2 기존 품질. 숫자를 사람 검수 완료로 해석하지 않음 |
| 운영·웹 | 서버 표적 배포와 로그 메트릭3개 생성·설정 검증 완료. 일반 웹 페이지 이동 측정 후속 #390은 공개 범위 검토 중 | 알림 수신인·정책, 실제 앱 이벤트 수신, 웹 배포 범위 응답, Crashlytics/ANR/진행 유실·14일 적격 관측 |

Android reference 기기의 다운로드 추정483MB, iPhone16의 다운로드500MB·설치620MB는 콘솔 추정치다. 실제 기기 측정이나 S5의 기본 모듈150MB 목표 달성이 아니다. #367은 다운로드 동의·Wi-Fi/모바일·오프라인 재실행·팩 삭제·원본 바이트 및 실제 크기 증거 전까지 HOLD한다.

현재 증거는 `C:/dev/hangulsori/_codex_artifacts/cp2026-release-stabilization-20260922/`의 `android7653-publication-receipt.json`, `ios253-publication-receipt.json`, `pr397-main-gate.json`, `pr401-main-gate.json`, `native-phase-main397-dispatch.json`, `ocr-copula-proof.json`, `content-coverage-main397.json`, `android7653-size-baseline.json`, `ios253-size-baseline.json`과 `jin-review/CHECKLIST.md`다. C01–C05 사람 검수, D01–D08 기기 검수와14일 품질 판정은 미완이다. 전체 진행률은 이 서로 다른 증거를 임의 가중해 백분율로 만들지 않는다.
