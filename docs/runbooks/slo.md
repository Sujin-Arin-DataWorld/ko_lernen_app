...
2026-09-23 기준 알림 정책·수신 채널 적용과 실제 수신은 미완이다.
정책 JSON이나 메트릭 설정의 존재만으로 알림이 운영 중이라고 판단하지 않는다.
9월 22일 확인한 Android Crashlytics는 지표 분모가 없는 상태였고 기존 iOS는
시작 안내 화면이었다. 빈 대시보드를 오류 0 또는 크래시 프리 100%로 읽지 않는다.
실제 동의한 내부 빌드의 SDK 수신과 적격 관측 시작일을 확정한 뒤 14일을 계산한다.

| SLO | 목표 | 측정 방법 | 누가, 어떻게 |
|---|---|---|---|
| 크래시 프리 세션 | ≥ 99.5% (동의한 코호트만, 14일 롤링) | Firebase Crashlytics → Dashboard → Crash-free sessions, 기간 14d | 실제 동의와 SDK 수신이 확인된 세션의 분모·빌드·기간을 기록한다. 익명 로그인 여부 자체가 제외 조건은 아니며 미동의 사용자는 수집하지 않는다. 사용자 비율과 세션 비율을 혼용하거나 관측 코호트를 전체 사용자 대표값으로 확대하지 않는다. |
...
| 함수 5xx율 | < 1% | `run.googleapis.com/request_count`, `response_code_class="5xx"` / 전체, 서비스별 | **알림 정의 있음, 실제 적용·수신은 미완**(2% 임계치, `tool/ops/alert_policies/01_functions_5xx_rate.json`). 알림 임계치(2%)는 SLO 목표(1%)보다 느슨하다. 실제 정책을 적용한 뒤에도 알림 없음이 SLO 충족을 뜻하지 않는다. SLO 실측은 Cloud Monitoring Metrics Explorer에서 기간·서비스·분모와 함께 확인한다. |
