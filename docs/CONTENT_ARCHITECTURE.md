...
Batch 06의 첫 C1/C2 scenario는 현재 review-only draft다. preview에서 전체 그래프를
통과해도 live 데이터로 계산하지 않는다. Jin 승인과 전용 scenario transaction이 끝난 뒤에만
위 데이터 부재 센서와 exact 선택 회귀를 갱신한다.
