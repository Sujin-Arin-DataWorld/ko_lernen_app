...
6. **§19 그랜드파더 allowlist 정리 — `hero_placement_guard`·`chrome_stack_guard`** — (a) `hero_placement_guard` 유예 4화면(chosung_quiz_screen/hangul_screen/legacy_vocab_screen/kkeunmari_screen)에서 `HanokHeader`를 제거하고 allowlist를 4→0으로 좁힌다. (b) `chrome_stack_guard`의 `chipWrapAllowlist`: 기존 4화면(chosung_quiz_screen 4/hangul_screen 2/legacy_vocab_screen 2/scenario_player_screen 2)에 **2026-08-27 W3↔main 병합에서 `study_library_screen.dart`(3개 — 뷰 선택 Wrap, 저장 유형 필터 Wrap, 항목별 상태 배지 Wrap)가 추가돼 백로그가 4→5화면으로 늘었다.** 이 화면은 main의 온보딩-미디어 웨이브가 `chrome_stack_guard`보다 먼저 작성했다 — 그 브랜치 관점에서는 규칙이 아직 없어 어길 수 없었던, 나머지 4곳과 동일한 성격의 그랜드파더(리팩터 유예이지 신규 위반 승인이 아니다). 5화면 모두 `SoriChromeRow`/`SoriLevelFilterBar` 단일 행으로 리팩터해 allowlist를 5→0으로 좁힌다.
