...
## Task 5: Add deletion-proof issuance and public consumption endpoint
...
- [ ] Write failing runtime tests for issueDeletionProof and requestDeletionByProof, including proof replay/expiry/response loss, bounded issuance/rotation, generic public response, secret-redacted logs/errors, and request-size/rate-hook rejection.
- [ ] Run: npm test -- --test account_operations_runtime.test.js
  Expected: proof endpoint behavior is absent or fails its safety assertions.
- [ ] Issue 256-bit proofs server-side, persist only a keyed hash/expiry/issuance metadata, and atomically claim or reuse an opaque operation ID in a Firestore transaction. Add a minimal public HTTP requestDeletionByProof endpoint that consumes no raw proof after validation, returns the same generic response for invalid/expired/used/deleted cases, applies bounded request limits, request-size limits, and records only safe metadata.
- [ ] Make proof issuance/consumption use only safe codes in logs/errors and add deployment configuration only for the new first-party endpoint, without deploying it. The endpoint may claim/create an operation but must not perform destructive cleanup in this task.
- [ ] Run: npm test. Expected: all Node tests pass.
- [ ] Commit: feat(functions): add server-owned deletion proof endpoint

## Task 6: Add destructive deletion worker and restrictive rules
...
- [ ] Write failing runtime tests for completeAppleRevocation, server-worker lease renewal, worker replay, Auth user-not-found, paged deletion continuation, existing Gye cleanup phase completion, and legacy client tombstone isolation.
- [ ] Run: npm test -- --test account_operations_runtime.test.js
  Expected: destructive worker/rules behavior is absent or fails its safety assertions.
...
- [ ] Remove client rule permission to create account_deletions or delete a user root; allow clients only the narrowly needed operation status reads for their own uid. Test that a worker-owned operation cannot be abandoned by legacy tombstone cleanup.
...
- [ ] Commit: feat(functions): add server-owned deletion worker and rules

## Task 7: Initialize App Check and replace client-side deletion with typed callable clients
...
## Task 8: Add typed remote reads and deterministic reconciliation
...
## Task 9: Migrate bookshelf sync to immutable generations and guard media cleanup
...
## Task 10: Implement anonymous collision coordination without accidental login
...
## Task 11: Route all account UI through safe operations and localize user-facing states
...
## Task 12: Publish a safe proof-consumption page and correct release documentation
...
## Task 13: Verify the complete branch, perform independent review, and prepare handoff
