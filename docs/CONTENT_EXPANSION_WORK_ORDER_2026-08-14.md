...
2. 17개 파일은 `reference_intake/source_inventory.csv`에서 SHA-256, 쪽수, 중복, text/mixed/image, OCR 필요 여부, 권리 상태로 관리한다.
...
9. scenario-only preview는 `integrate_scenario_batch.py --manifest ...`로 수행한다. Jin 승인 전 `--apply`, TTS, Firebase, asset append는 금지다.
...
5개다. quest 유형은 `hoerverstehen`, `uebersetzen`, `luecken`, `satzBauen`, `diktat`이며
총 4개 scenario와 20개 실제 게임 항목이다. C1/C2 시나리오 0개 공백을 먼저 해소하되
B1/B2도 같은 계약을 통과하는 회귀 표본을 포함한다.
