...
- 위 clean-room 규칙에는 하나의 예외가 있다. 공공누리 제1유형(출처표시) 조건으로 공개된
  공공저작물은 라이선스가 `source_inventory.csv`에 `rights_status=licensed`,
  `allowed_use=licensed_transform`로 기록된 경우에 한해 원문을 그대로 학습 콘텐츠로 쓸 수
  있다. 이때도 manifest `provenance.rights="kogl_type1"`·`provenance.sources`(저작물
  제목·발행처)와 앱 설정 › 출처·라이선스 화면의 고지가 함께 필요하다. 조건과 절차의
  정본은 `CONTENT_SOURCE_POLICY.md`의 "공공누리 제1유형 자료의 예외"다. 라이선스가
  인벤토리에 기록되지 않은 자료는 이 예외를 쓸 수 없다.
...
레벨 기준의 정본은 `docs/CONTENT_LEVEL_BIBLE.md`다. 아래 표는 요약이다.

| 레벨 | 쓰는 상황 | 언어적 목표 | 피해야 할 것 |
| --- | --- | --- | --- |
| A1 | 자기소개·직업·국적·장소·전화번호·위치, 수량 주문·음식 취향, 권유·요일/날짜, 과거 활동·계획, 높임 기초·약속 제안, 날씨·묘사·이유/불가능(세종 1~14과) | 국제통용 1급 문법 45항목 전부(조사·선어말어미·연결어미·종결어미·표현), 1급 어휘 735(생활 고유어·기본 동사/형용사·수사·단위명사), 해요체 기본 + 합쇼체 인사·공식 표현 인지, 문장 ≤8어절·절 1~2개 | 1급 밖 단어를 문장당 2개 이상 쓰는 것, 1급 밖 문법(‑으려고·‑을 수 있다 등을 B1처럼 늦게 배치), 절 3개 이상으로 문장 늘리기 |
| A2 | 취향·이유, 약속 제안·변경, 메신저, 증상·감정, 교환·택배, 학업/직장 실수, 이동 불편 요청, 집안 문제 | 국제통용 2급 문법 45항목(‑는데·‑으면·‑거나·‑게 되다·‑는 것 같다 등), 2급 어휘 1,100, 반말 인지 + ‑습니다 생산, 문장 ≤12어절·절 ≤2개 | 2급 밖 단어를 문장당 2개 이상 쓰는 것, A1 단어를 그대로 두고 문장만 길게 늘리는 방식, B1 문법 조기 사용 |
...
ID의 level segment는 생성 시점의 레벨이다. 레벨이 이동한 항목은
`tools/content_factory/relevel_ledger.json`에 기록되며 검증기는 원장에 있는 ID에
