...
**C. Xcode Organizer**
`ios/ExportOptions.plist` 의 `destination` 이 `export` 라 스크립트는 업로드하지 않고
파일만 만든다(의도된 설계 — 실수 업로드 방지). Xcode 에서 직접 Archive 했다면
Organizer → Distribute App 으로도 올릴 수 있다.
...
GitHub Actions `macos-latest` 러너로 같은 스크립트를 돌릴 수 있다 — 인증서(.p12),
프로비저닝 프로파일, `GoogleService-Info.plist`, ASC API 키를 리포지토리 Secrets 에
넣고 워크플로에서 복원하는 방식이다. 필요해지면 그때 워크플로를 추가한다.
