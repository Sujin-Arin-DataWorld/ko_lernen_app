...
- [ ] ⚠️ **그 이후 추가된 Dart 변경**(`definitionKo` 필드 + **"나만의 단어장" 신규 화면 2개·모델·서비스·신규 l10n 30키**)이 있어 **반드시 `flutter gen-l10n` → `flutter analyze` → `flutter test` 순서로 1회**. 신규 l10n 키는 생성 파일이 stale이라 gen-l10n 전엔 analyze 실패가 정상.
