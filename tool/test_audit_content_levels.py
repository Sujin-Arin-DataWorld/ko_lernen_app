...
* ``FixtureAuditTest`` builds a tiny, fully hand-computable content tree
  (2 vocab packs / 4 words, 2 grammar rows + 1 flagged, 1 scenario, 2 cloze,
...
def build_fixture(root: Path) -> None:
    ...
    vocab_rows = [
        ...
        # this median/share (R4 item 4: "pack medians ... use high-
        # confidence word grades only"), so adding it changes n_words
        # (4->5) but leaves median_delta/share_ge_plus2 unchanged; n_high
        # stays 4.
        ...
        # delta_pack=+2.0 (bundle_move override)
        ...
        # from the pack median>=2 bundle_move override" decision: pack3's
        # median (high-conf only, i.e. just 정책's grade 5) is 5.0,
        # delta_pack=+4.0 >= 2 -> override fires, but must only touch 정책
        # (word_move -> bundle_move), never 사전 (stays review_fallback).
        vocab_row("사전", "A1", "a1_test_pack_3", 1, "vocab_a1_0007"),   # fallback_over2, exempt from override
        vocab_row("정책", "A1", "a1_test_pack_3", 2, "vocab_a1_0008"),   # g5 delta+4 over2, overridden to bundle_move
    ]
    ...
    grammar_rows = [
        ...
        # sentence-level fallback_over2 (R4 item 4): example_korean is the
        # single low-confidence token 사전 (basic2023 grade 6 capped to 3
        # for lexical_p90 purposes) -> base_grade=3 (B1), delta = 3-1 = +2
...
class FixtureAuditTest(unittest.TestCase):
    ...
    def test_fallback_over2_exempt_from_pack_override(self):
        ...
        self.assertEqual(it.bucket, "fallback_over2")
        # pack3's high-confidence-only median (4.0, from 정책 alone) is
        # >=2 and DOES trigger the pack override below, but a
        # fallback_over2 row must stay 'review_fallback' -- overriding an
        # explicit "our own confidence is shaky, a human must look" signal
        # with a blanket bundle_move would defeat its purpose (R4 item 4
        # design decision, documented on apply_pack_overrides).
        self.assertEqual(it.suggested_action, "review_fallback")
        self.assertIn("fallback:basic2023", it.reason)

    def test_pack3_override_still_applies_to_high_confidence_sibling(self):
        it = self.by_kind_id[("vocab", "vocab_a1_0008")]  # 정책 in pack3
        self.assertEqual(it.bucket, "over2")
        self.assertEqual(it.confidence, "high")
        self.assertEqual(it.suggested_action, "bundle_move")

    def test_pack2_median_ge_plus2_forces_bundle_move_on_both_words(self):
        ...
        # both overridden to bundle_move because pack2's median delta is 2.0
        self.assertEqual(it_low.suggested_action, "bundle_move")
        self.assertEqual(it_high.suggested_action, "bundle_move")
        self.assertEqual(it_low.blocked_by, "can_do_ref")
        self.assertEqual(it_high.blocked_by, "can_do_ref")

    def test_pack_stats(self):
        # R4 item 4: n_words counts every pack word; n_high/median_delta/
        # share_ge_plus2 count HIGH-CONFIDENCE words only. pack1 gains a
        # 5th word (사전, low-conf) but n_high/median/share are unchanged
        # from the pre-R4 4-word numbers -- proof the low-conf word is
        # excluded, not just coincidentally absent.
        p1 = self.result.pack_stats["a1_test_pack_1"]
        self.assertEqual(p1.n_words, 5)
        self.assertEqual(p1.n_high, 4)
        ...
        self.assertEqual(p3.n_high, 1)  # only 정책; 사전 (low-conf) excluded
    ...
    def test_matrix_vocab_a1_row(self):
        ...
        # the suspects bucket) + pack3 (사전 g6, 정책 g5).
    ...
    def test_summary_counts_and_packs_and_coverage(self):
        ...
        self.assertEqual(
            ...
            {"over2": 3, "over1": 2, "under2": 0, "unknown": 0, "fallback_over2": 2, "total": 10},
        )
        self.assertEqual(
            summary["counts"]["grammar"],
            {"over2": 1, "over1": 0, "under2": 0, "unknown": 0, "fallback_over2": 1, "total": 4},
        )
        self.assertEqual(
            ...
            summary["packs"],
            {
                "a1": {
                    ...
                    "share_ge_plus2_top10": [
                        {"pack_id": "a1_test_pack_3", "median": 4.0, "share_ge_plus2": 1.0, "n_high": 1},
                        {"pack_id": "a1_test_pack_1", "median": 0.5, "share_ge_plus2": 0.25, "n_high": 4},
                    ],
                },
                "a2": {
                    "median_ge_plus2": 1,
                    "share_ge_plus2_top10": [
                        {"pack_id": "a2_test_pack_1", "median": 2.0, "share_ge_plus2": 0.5, "n_high": 2},
                    ],
                },
            },
        )
        self.assertEqual(
            ...
            {"total_unique": 3, "present_in_app": 2, "at_level": 2, "missing": 1},
        )
        self.assertEqual(
    ...
    def test_coverage_missing_word_grouped_by_pos(self):
        ...
        self.assertEqual(cov.missing_words_by_pos, {"명사": ["친구"]})
    ...
    def test_write_suspects_csv_header_and_row_count_and_sort_order(self):
        ...
        body = rows[1:]
        # 7 vocab + 2 grammar (hard + fallback) + 1 scenario + 1 cloze +
        # 1 satz + 1 smalltalk + 2 pronunciation (over2 + unknown) + 1 media
        # = 16 total.
...
class LiveRatchetTest(unittest.TestCase):
    ...
    lower-only caps (2026-09-07 R4 baseline: confidence-aware counting
    splits a chunk of the old over2 into fallback_over2, and the lexicon
    recalibration + proper-noun/number "not unknown" fix both lower
    vocab's unknown ratio — every CAP below is a genuine improvement over
    the pre-R4 baseline, not just a renumbering). Run
    `python tool/audit_content_levels.py` first to regenerate the summary."""

    # 2026-09-07 R4 실측 (`python tool/audit_content_levels.py --json` 출력).
    # 하향 전용 — 상한은 내려갈 수만 있다, 절대 올리지 마라. over2 is HIGH-confidence-only
    # by construction now (R4 item 4) -- a non-high >=+2 verdict counts
    # under CAP_FALLBACK_OVER2 instead, see below.
    CAP_OVER2 = {
        "vocab": 220, "grammar": 9, "scenario": 6, "cloze": 221,
        "satz": 253, "smalltalk": 57, "pronunciation": 8, "media": 17,
    }
    # 실측 unknown/total: vocab .0240, 나머지 0 (R4: proper-noun/number
    # tokens are no longer misclassified as unknown, and the recalibrated
    # lexicon resolves more directly -- both pronunciation/media dropped to
    # 0 from a nonzero pre-R4 baseline). +0.01 여유는 브리프 지시(래칫 조건)
    # 그대로.
    CAP_UNKNOWN_RATIO = {
    ...
    # R4 item 4 (new bucket) 실측 2026-09-07.
    CAP_FALLBACK_OVER2 = {
        "vocab": 151, "grammar": 3, "scenario": 1, "cloze": 39,
        "satz": 36, "smalltalk": 7, "pronunciation": 0, "media": 6,
    }
    ...
    def test_pack_top10_entries_have_expected_shape(self):
        for level in ("a1", "a2"):
            for entry in self.summary["packs"][level]["share_ge_plus2_top10"]:
                self.assertEqual(
                    set(entry), {"pack_id", "median", "share_ge_plus2", "n_high"},
                )
