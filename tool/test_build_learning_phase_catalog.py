...
class LearningPhaseProjectionTests(unittest.TestCase):
    def test_live_projection_is_fresh_and_preserves_canonical_goals(self):
        ...
        self.assertEqual(len(actual["phases"][0]["taskIds"]), 31)
        self.assertEqual([len(p["taskIds"]) for p in actual["phases"][:4]], [31, 29, 29, 27])
        self.assertEqual([len(p["taskIds"]) for p in actual["phases"][4:8]], [17, 22, 20, 21])
        self.assertEqual(actual["phases"][8]["taskIds"], [])
