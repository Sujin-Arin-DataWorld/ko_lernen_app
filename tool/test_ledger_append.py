...
class ValidateSyntheticRuleTest(unittest.TestCase):
    ...
    def test_an_approved_output_becomes_a_known_input_for_later_records(self) -> None:
        with tempfile.TemporaryDirectory(prefix=".ledger-test-", dir=ROOT) as tmp:
...
class AppendRecordTest(unittest.TestCase):
    def test_append_computes_hashes_and_writes_a_valid_record(self) -> None:
        with tempfile.TemporaryDirectory(prefix=".ledger-test-", dir=ROOT) as tmp:
            ...
            asset_rel = asset.relative_to(ROOT).as_posix()

            ledger_path = tmp_path / "ledger.json"
            ledger_path.write_text(
                json.dumps(_minimal_ledger(
                    [], allowed_inputs=[{"path": asset_rel, "sha256": asset_sha}]
                )),
                encoding="utf-8",
            )
            ...
            spec_path.write_text(
                json.dumps({
                    ...
                    "inputAssets": [{"path": asset_rel}],
                    "outputAssets": [{"path": output_rel, "decision": "approved", "kind": "part"}],
                }),
                encoding="utf-8",
            )
