...
변경 전 전체 기준선도 별도로 확인했다: `5220 passed / 14 skipped / 0 failed`. 이미지 작업 뒤에는 이번 변경 범위와 직접 연결된 위의 한옥 매니페스트·턴테이블 테스트를 다시 실행했다.

## 남은 승인 게이트

자동 검증과 제작자 육안 검토는 통과했지만, 최종 사람 시각 승인과 실제 게임 배치 승인은 아직 열려 있다. 그 승인 전에는 런타임, 카탈로그, Firebase, 릴리스 자산으로 승격하지 않는다.
