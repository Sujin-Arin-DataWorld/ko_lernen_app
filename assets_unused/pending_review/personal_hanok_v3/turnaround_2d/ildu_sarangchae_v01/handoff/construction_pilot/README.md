...
`implementation_snapshot/`의 Dart, 테스트, JSON은 당시 시범을 이해하기 위한 자료다. 두 Dart 파일은 저장소 분석 대상에서 제외되도록 원래 파일명 뒤에 `.snapshot`을 붙여 보존했다. 다음 세션은 먼저 최신 `main`의 동일 기능 존재 여부와 저장 계약을 검색해야 한다. 파일을 그대로 덮어쓰거나 현재 런타임 권위라고 가정하지 않는다.
