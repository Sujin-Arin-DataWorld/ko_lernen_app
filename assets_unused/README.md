...
## 5. 검토 대기 `pending_review/` (2) — 2026-08-07
...
합계 **4.2MB** 가 AAB 에서 빠진다. AAB 기기별 다운로드가 190MB/200MB 라
여유가 10MB 뿐이었으므로 무시할 크기가 아니다.

> **주의 — `assets/video/character/` 는 단방향 가드다.** `character_clip_matte_test.dart:77`
> 은 "상수가 가리키는 파일이 실재하는가"만 보고 그 반대(디스크에 있는데 상수에
> 없는 것)는 안 본다. 그래서 `tiger_magpie_play.mp4` 가 오래 남아 있었다.
> 양방향 전환은 커밋 5(가드)에서 다룬다.
