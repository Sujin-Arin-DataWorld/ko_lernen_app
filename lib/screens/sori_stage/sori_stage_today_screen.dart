...
class _TodayMissionStage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 오늘 할 활동 자체 — 제목·일러스트의 출처. 조회는 기존 라우트 매핑을
    // 그대로 쓴다(신규 조회 함수 발명 금지).
    final ActivityCatalogEntry? entry = activityForRoute(destination?.route);
    final List<RewardContractItem> rewards = contract?.items ?? const [];
    return Container(
      // 상단 배너가 카드 모서리까지 꽉 차야 하므로 패딩은 안쪽 Padding 이 맡는다.
      ...
      child: Column(
        ...
        children: [
          // 오늘의 주인공을 무대에 올린다. entry 가 없으면 배너 자체를 만들지
          // 않는다 — AspectRatio 는 자식이 아니라 제약으로 크기가 잡히므로
          // errorBuilder 만으론 빈 다크 밴드가 남는다 (1차 강등은 이 게이트).
          if (entry != null)
            AspectRatio(
              aspectRatio: 21 / 9,
              child: Image.asset(
            ...
            child: Column(
              ...
              children: [
                Text(
                  ...
                  // 짙은 한옥 스테이지 위라 석간주 대신 골드.
                  style: tt.eyebrow.copyWith(color: SoriColors.gold),
                ),
                const SizedBox(height: Spacing.sm),
                Text(
                  // 제목이 **오늘 뭘 하는지**를 말한다. 예전엔 eyebrow·제목·CTA
                  // 셋이 같은 ARB 키라 "오늘의 미션 시작"이 두 번 반복됐다.
                  entry == null
                      ? (destination == null
                            ? t.soriStageTodayEmpty
                            : t.soriStageMissionAction)
                      : localCopy(context, entry.title),
                  // 카드 내부 헤드라인은 h1 상한 — hero(38)는 페이지 헤더 전용.
                  style: tt.h1.copyWith(color: Colors.white),
                ),
                if (rewards.isNotEmpty) ...[
                  const SizedBox(height: Spacing.lg),
                  // 보상은 ' · ' 조인 문자열이 아니라 아이템별 칩 — 종류마다
                  // kind 가 다르므로 단일 아이콘으로 뭉뚱그릴 수 없다.
                  Wrap(
                    spacing: Spacing.sm,
                    ...
                    children: [
                      for (final item in rewards)
                        _RewardChip(
                          icon: soriRewardIcon(item.kind),
                          label: localCopy(context, item.label),
                        ),
                    ],
                  ),
                ],
...
class _RewardChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: [
        Icon(icon, size: 16, color: SoriColors.gold),
        const SizedBox(width: Spacing.xs),
        Text(
          label,
          style: tt.label.copyWith(color: SoriActivityColors.onHanokStage),
        ),
      ],
    );
  }
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // 아이콘 하나 대신 **지금 지어진 한옥 그림**을 보여 준다.
            // 자산 매핑은 기존 리졸버(HanokStage.assetSlug)를 그대로 쓴다.
            AspectRatio(
              aspectRatio: 16 / 5,
              child: Stack(
                fit: StackFit.expand,
                children: [
                  ...
                  Positioned(
                    ...
                    child: Text(
                      '$built / $total',
                      // 진행 수치는 tabular — 조각이 늘어도 자리 흔들림 없음.
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: Spacing.sm),
                  Text(
                    // enum 원문("empty")이 화면에 노출되던 자리 — 공용
                    // 라벨 매핑으로 교체했다.
                    '${t.soriStageNextPiece}: '
                    '${soriHanokStageLabel(t, stage)}',
                    style: tt.label,
                  ),
                ],
              ),
...
class _QuestProgressRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final tt = SoriTextTheme.of(context);
    // 맨 ListTile → 카드 규율. 보상 썸네일이 "무엇을 얻는지"를 그림으로
    // 말해 준다 (퀘스트 화면과 같은 시각 언어).
    return Padding(
