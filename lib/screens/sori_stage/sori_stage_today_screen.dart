...
class _TodayMissionStage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Preserve unavailable early-return if a parallel session adds it —
    // empty / degraded missions must not look like a startable poster.
    final t = AppL10n.of(context);
    ...
    final items = contract?.items ?? const <RewardContractItem>[];
    final radius = BorderRadius.circular(SoriRadius.xl);
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: Spacing.sm),
                Text(title, style: tt.h1.copyWith(color: Colors.white)),
                if (items.isNotEmpty) ...[
                  const SizedBox(height: Spacing.lg),
                  Wrap(
                    spacing: Spacing.sm,
...
class _MissionRewardChip extends StatelessWidget {
  const _MissionRewardChip({required this.icon, required this.label});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          Icon(icon, size: 16, color: SoriActivityColors.onHanokStage),
...
class _HanokProgress extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final stageName = hanokStageLocalizedName(t, stage);
    return InkWell(
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: Spacing.sm),
                  Text(
                    '${t.soriStageNextPiece}: $stageName',
                    style: tt.label,
                  ),
                ],
              ),
