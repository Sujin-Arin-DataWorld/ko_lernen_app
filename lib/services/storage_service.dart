...
class Storage {
  ...
  static String? recentCatalogActivityId(SoriStageTab tab) {
    ...
    try {
      final value = jsonDecode(raw);
      if (value is! Map ||
          value['uid'] != cloudWriteSessionController.current?.uid) {
        return null;
      }
      final id = value['id'];
      return soriActivityCatalog.any((e) => e.id == id && e.tab == tab)
          ? id as String
          : null;
    } catch (_) {
      return null;
  ...
  static Future<void> _enqueueCatalogHistory(
    ...
    if (_catalogHistoryResetting > 0 || !lease.isCurrent) {
