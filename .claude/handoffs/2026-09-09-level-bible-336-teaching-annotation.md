...
# 부록 — 설계 실측 (2026-09-09 조사 완료, 다음 PR 은 이 조사를 반복하지 마라)

아래 수치는 전부 이 워크트리에서 직접 돌려 얻었다.

## 실측표

| 사실 | 값 |
|---|---|
| `ko.json` 왕복 무손실 | `json.dumps(d, ensure_ascii=False, indent=2) + "\n"` 이 원본과 **바이트 동일** |
| `forms[]` 키 | 정확히 `{form, category, variants, meaning}` 4개 |
| `form` 단독 유일성 | **아니다** — `-고4`·`-는다고1`·`-다니1` 3건이 급을 넘어 중복 |
| `(grade, form)` 유일성 | 336/336 |
| `nikl_g{grade}_{_slug(form)}` 유일성 | **336/336**, 게다가 `GrammarIndex` 의 `pattern_id` 와 같은 형식 |
| `meaning` 채워진 수 | CSV 91 / `ko.json` 91 (동일) |
| 앱 문법 카드에서 씨앗을 받을 수 있는 형태 | **139**/336 (matched app id ≥1) |
| `assets/data/grammar.csv` 252행 | `type_de/en`·`explanation_de/en`·`example_korean/german/en`·`note/note_en` **전부 252/252 채워짐** |
| 예문 재사용 풀 | 앱 전 코퍼스 고유 한국어 문장 **4,350** |
| **기계 검증 통과 재사용 가능 형태** | **205/336** (형태를 실제로 포함 + `sentence_profile().level_estimate ≤ 형태 급` + 형태마다 서로 다른 문장으로 이분매칭) |
| 재사용 출처 | grammar.csv 122 · korean_vocab 65 · 시나리오 16 · cloze 2 |
| **신규 집필 필요 예문** | **131** (급별 9/9/25/29/35/24 · 범주별 표현 80·종결 17·연결 16·조사 13·선어말 4·전성 1) |
| DE/EN 무료 획득 | 재사용 205개는 3언어가 이미 채워져 온다. 신규 131개만 DE/EN 신규 |
| `GrammarIndex` 규칙이 있는 형태 | **186/336** — 조사 60·전성어미 7·선어말어미 4 전부와 짧은 파편은 규칙이 안 만들어진다 |
| CI 스코프 | `docs/**`·`*.md` **단독 변경은 CI를 아예 안 띄운다**(`.github/scripts/ci_scope.py:226-231`). 배치는 항상 `ko.json` 을 함께 바꾸므로 문제없다 |

**함의 둘.** (1) `ko.json` 이 indent=2 로 왕복 무손실이므로 배치 집필을 스크립트가 로드→수정→덤프로 하면
헛디프가 0이다 — 손으로 JSON 을 편집할 이유가 없다. (2) `GrammarIndex` 검출 가능 형태가 186개뿐이라
"예문이 그 형태를 담고 있는가" 검사는 **2단**이어야 한다(GrammarIndex 186 / 정규식 150).

## 설계 결정 (갈림길과 이유)

| 갈림 | 선택 | 이유 |
|---|---|---|
| §B 물질화 | **생성 구간 마커** `<!-- BEGIN GENERATED: B.n.③ … -->` | Jin 지시가 "§B 의 목록 부분을 생성기가 만들게"이지 이관이 아니다. A1·A2 인라인 표를 링크로 내리는 것은 문제의 악화다. 선례가 없으므로 계약을 테스트 3종(멱등성 · 마커 밖 무변화 · 마커 파손 시 raise)으로 못 박는다 |
| 증거 등급 표현 | **프로파일 id 한 줄 + `ko.json` 루트의 등급표 + 감사기 재계산** | 항목당 8키 맵은 3,000줄 상용구다. 프로파일은 문서화용이고 권위는 `audit_learning_phases.evidence_for(axis, sources)` 재계산이다(불일치는 error) |
| 등급 어휘 | **새로 만들지 않는다** — `audit_learning_phases.py` 의 `EVIDENCE_TAGS` 3값 재사용, 기존 4값 `provenance` 와는 축을 분리해 병존 | 세 번째 어휘가 생기면 아무도 못 지킨다 |
| provenance ↔ evidence | 크로스워크를 불변식으로: `verified_repo ⇒ OFFICIAL 허용`, 나머지 3값 `⇒ DERIVED\|PEDAGOGICAL 만` | "뜻·기능·예문을 OFFICIAL 로 올리는 것 금지"를 테스트 가능한 형태로 고정 |
| **앱 문장 재사용 시 등급** | 재사용해도 **등급은 오르지 않는다.** `example.source` 에만 적고 evidence 는 PEDAGOGICAL 유지 | 재사용 문장은 우리가 쓴 문장이지 공식 인벤토리가 아니다. "저장소에 있으니 verified_repo" 라는 승급을 프로파일 note 로 막는다 |
| F11 이 `build_f1` 재사용 | **안 한다** | `assets/data` 의존이 생겨 신선도 게이트가 불가능해진다. 게다가 F1b 가 F1 매처 오분류 6건을 실명으로 기록해 뒀다. 단 **씨앗 추출(일회성)에는 쓴다** — 139개 형태의 DE/EN 초안이 공짜다 |
| 예문 출처 | **앱 재사용을 기본값** (205/336) | 이미 검수를 통과했고 DE/EN 이 딸려 오며, `example.sourceText` drift 검사로 어긋남이 기계로 잡힌다. 바이블 예문 = 앱 예문이면 학습자가 바이블에서 본 문장을 앱에서 그대로 만난다 |
| 래칫 | **`CAP_TEACHING_GAP_TOTAL` 별도 카운터**(초기 1,680 = 336×5), `CAP_GAP_TOTAL`(현재 560)은 손대지 않는다 | "캡은 내리기만" 규칙을 지키면서 배치가 순차 머지되는 동안 CI 를 계속 초록으로 유지 |
| 신선도 게이트 범위 | **F11·F12·F13 + §B 생성구간만** | F1/F2/F3/F5/F7 은 `assets/data` 의존이라 콘텐츠 PR 마다 낡고 F6 는 기계 로컬 소스 의존이다 |

## 예문 기계 검증 — 재사용할 함수

| 검사 | 함수 | 판정 |
|---|---|---|
| 레벨 상한 | `tool/cefr_lexicon.py:2429 sentence_profile` → `level_estimate` | 형태 급 초과 ⇒ error |
| 길이·절 수 | `cefr_lexicon.py:193 LENGTH_LIMIT_BY_GRADE`, `:1667 apply_length_rule`, `:1621 clause_count` | §B ⑤ 상한 초과 ⇒ error |
| 형태 포함 1단 | `cefr_lexicon.py:2831 GrammarIndex.detect` — `pattern_id` 를 `_v\d+$` 제거 후 form id 와 비교 | 186개 형태 |
| 형태 포함 2단 | `cefr_lexicon.py:2612 compile_pattern_regex` — form + variants 전개 | 나머지 150개 |
| 자연성 마커 | `tool/audit_content_naturalness.py:237 check_generic_markers` | 히트 ⇒ warn |
| 인물명 금지 | `cefr_lexicon.py:1416 load_character_names` | 세종 교재 인물명 ⇒ error |
| 앱 드리프트 | `example.sourceText` ≠ 앱 레코드 실텍스트 | ⇒ error |

완화 규칙 하나: 정규식 경로(150개)에서 `estimate_grade < 형태 급 - 1` 이면 **info** 로만 본다 — 조사·전성어미는
문장 급을 스스로 못 올리므로 error 로 삼으면 오탐만 난다.

## 집필 분할 — 26배치, 1급부터

등급 × 범주, 상한 20, 꼬리 셀은 같은 급 인접 배치에 합친다.

```
g1: 선어말3+연결6(9) · 조사19 · 종결8+표현9(17)                  → 3배치 / 45
g2: 연결6+전성6(12) · 조사10 · 종결6 · 표현17                    → 4배치 / 45
g3: 연결15+선어말1+전성1(17) · 조사10 · 종결7 · 표현17 · 표현16   → 5배치 / 67
g4: 연결17 · 조사13 · 종결11 · 표현13 · 표현13                    → 5배치 / 67
g5: 연결7+조사3(10) · 종결10 · 표현18 · 표현18                    → 4배치 / 56
g6: 연결16 · 조사5 · 종결20 · 표현15                              → 5배치 / 56
```

**1급부터 시작한다** — 45개 중 40개가 F1 `match` 라 앱 씨앗이 가장 두껍고, 레벨 상한이 가장 빡빡해서
(≤8어절, 1급 어휘만) 검증기가 가장 빨리 실패 신호를 준다. 5·6급 표현(36+15)을 마지막에 둔다 —
씨앗이 거의 없고(표현 87개가 재사용 불가) 가장 어렵다.

배치마다: 스크립트 집필 → **기계 게이트가 초록이 된 뒤에** 적대적 검증 → 수정 루프 → 래칫 하향.

## 적대적 검증에서 쓸 공격 4종

| 공격 | 방법 |
|---|---|
| **기계가 놓친 레벨 위반** | `sentence_profile` 의 `allowance` 규칙이 최고급 토큰 **1개를 90퍼센타일에서 제외**한다(`cefr_lexicon.py:1637`). 그 면제 토큰을 뽑아 "이 예문이 사실 상위 어휘 하나에 기대고 있는가"를 사람이 본다 |
| **뜻과 형태 불일치** | 동형어를 겨눈다 — `-고3`(나열) vs `-고4`(덧붙여 서술), `은1`(조사) vs `-은2/-은3`(전성어미), `까지`. CSV `meaning` 태그가 있는 91개는 태그와 대조하고, 나머지 245개는 뜻을 형태에 되먹여 "이 뜻으로 이 형태가 유일하게 지목되는가"를 본다 |
| **대조가 공허함** | **치환 테스트** — `contrasts[].with` 형태를 예문에 바꿔 넣는다. 문장이 여전히 자연스럽고 뜻도 여전히 들어맞으면 그 대조는 공허하다 |
| **예문이 형태를 드러내지 않음** | 형태를 지우고 읽어서 문장이 멀쩡하면 반려(예: `까지` 예문인데 요지가 `부터`) |

반려는 `docs/data/level_bible/F10_review_lessons.md` 에 "form_id · 문제 · 이유 · 고친 예"로 누적한다.
같은 사유 2회 → 다음 배치 브리프의 금지 패턴에 추가. 5회 초과 → 배치 분할.

## 실행 순서

S1 스키마 선언(`evidenceProfiles` + 336개 `id` 파생) → S2 주제 17×85 → S3 검증기·F11~F13·`--check`·
§B 스플라이서(**teaching 없이도 동작**, 전부 결손으로 렌더) → S4 감사 검사 → S5 테스트(이 시점
`CAP_TEACHING_GAP_TOTAL = 1680`, 나머지 캡 0) → S6 §B 마커·배너·§F 행 → S7 문서 → S8~S33 배치 26개.

**S1~S7 을 기반 PR 하나로, S8 이후를 배치 PR 로 쪼갠다.** 기반 PR 이 먼저 초록이 되면 이후 배치는
캡 하향만으로 진행 증거가 남는다.

---

# 부록 2 — B1 출처 충돌 조사에서 나온 실측 (2026-09-10)

PART 2(출처 품질 관리)를 위해 B1 대조표의 `disagreements` 12건을 집필하는 과정에서 원자료를
직접 세어 얻은 값들이다. 앞의 "모순 6건"을 강화하거나 새로 추가한다. 전부 재현 가능하다.

## 새로 확인된 것

- **공식 3급 67항목 중 32항목은 앱에 어떤 레벨로도 행이 없다.** 앱 B1 문법은 35행인데 3급과의
  교집합은 12행뿐이고, 나머지는 4급 8·5급 1·2급 1·336 목록 밖 13이다. 즉 F1 의
  `missing_in_app 197` 은 레벨별로 고르게 퍼진 게 아니라 B1 에서만 32개가 빠진 것이다.
- **앱 문법 행 32개는 id 접두사와 `level` 열이 서로 다르다.** 그중 `grammar_b1_*` 인데
  `level` 이 A1/A2 인 행이 **15개**다. 그 15행 중 실제로 3급 항목인 것은 `-어야`
  (`grammar_b1_obligation`) 하나이고 그 행의 level 은 A1 이다. `V-느라고` 는 A2·B1 두 행으로
  중복 존재한다. → 바이블 §E(재분류 정책)의 "id 불변 + 원장" 규칙이 있으므로 id 를 고치는 게
  아니라 `level` 을 바로잡고 원장에 남기는 문제다.
- **"67항목"은 형태 수가 아니라 뜻 갈래 수다.** 3급 7항목이 다른 급과 형태를 공유하고,
  `-다가1(2)`(3급 중단)은 `-다가1(1)`(2급 중단)과 **변이형·뜻 표기까지 동일**하다.
  → 그래서 `ko 67 / en 17 / de 15` 같은 **항목 수 직접 비교는 무효**다. 다음 PR 의 F11 은
  이 사실을 표에 명시해야 하며, 336 을 "고유 형태 336"으로 읽는 서술은 전부 틀렸다
  (고유 형태는 333, 그리고 급 안에서도 뜻 갈래가 겹친다).
- **3급 어휘 수가 출처마다 다섯 값이다**: 1,655(연구보고서) · 1,690(바이블 실측 행) ·
  1,713(CSV 행) · 1,621(고유 표제어) · 1,554(F2 신규 집계). 앞의 모순 5번을 확장한다.
- **그리고 감사기가 3~6급을 아예 측정하지 않는다.** `tool/audit_content_levels.py:1103-1104`
  가 `compute_coverage` 를 **1·2급에만** 호출하고, `tool/content_level_summary.json` 도
  `grade1`·`grade2` 만 담는다. 즉 B1~C2 어휘 커버리지는 F2 표에 숫자로만 있고 **어떤 게이트도
  보지 않는다.** 다음 PR 에서 3~6급으로 확장하고 래칫을 붙일 것.
- **정보 상태 표지의 급수 역전**: 국립국어원 3급이 `-잖아`·`-거든2`·`-던데2`·`-는구나` 를
  요구하는데, 대응하는 영어 담화표지는 C1, 독일어 Modalpartikeln 은 C2 다. 영어권·독일어권
  학습자에게 이 넷은 B1 에서 **모어 자원 없이** 배워야 하는 항목이라는 뜻이다.
- **CEFR-J 열과 EGP 열이 B1 에서 충돌하는 행**: 62/62-1 · 73/73-1(CEFR-J B2.1 vs EGP B1) ·
  75 · 82 · 132(might) · 135(must) · 154(whether) · 160(so that) · 181 · 213.
  게다가 `en.json` 이 `cefrj:73-1·75·200·201·203` 에 `egp:B1` 을 함께 달았지만 해당 행의
  CEFR-J 밴드는 B1 이 아니다 — 우리 인용에도 오류가 있다는 뜻이므로 F11 집필 전에 바로잡는다.
- **간접화법의 3언어 비대칭**: 영어 항목 200/201/202/203 이 CEFR-J A2.1~B1.2 / EGP A1~B2 로
  흩어지고, 독일어는 `dass`+Indikativ B1 vs Konjunktiv I B2, 한국어는 backshift 자체가 없다.
- **TOPIK 3급 3,000~4,000 은 누적치**다(1~3급 고유 표제어 3,338과 견줄 값). 국제통용 3급
  1,655 와 직접 대응시키면 안 된다.

## 그래서 다음 PR 에서 추가로 할 일

- F11 에 **"뜻 갈래 vs 형태" 열**을 두고, 같은 형태가 여러 급/뜻으로 나뉘는 10항목
  (`-고4`·`-는다고1`·`-다니1` + 3급 7항목)을 명시한다. 항목 수 비교를 금지하는 주석도 함께.
- `app grammar.csv` 의 id 접두사 ≠ level 32행을 §E 원장 방식으로 정리한다(별도 배치 PR 권장).
- `audit_content_levels.py` 의 커버리지 계산을 3~6급으로 확장하고 래칫을 붙인다.
- `en.json` 의 `egp:` 라벨 오인용 5건을 CEFR-J CSV 와 대조해 고친다.
