...
Task 10 complete: PR #248 병합(merge c81850ee). 최종 형태 — 기록=graphify-out/memory 노드(Codex P2 수용, 스탬프 철회), 동승 픽스 2건(ci.yml dispatch unshallow 01a873d0, browserslist 4.28.8 1dbee4c2). PR CI run 1025 success(Website 게이트+풀스위트). 리뷰 스레드 전건 resolve.
Task 10 사후 확인: main@c81850ee push CI run 1026 success(20:27 UTC). Codex 오디오 신규 커밋 아직 없음(main tip 그대로) — 릴리스 대기 유지. 검수 대장 아티팩트 발행: https://claude.ai/code/artifact/d1b48c65-dd1e-4648-8bd5-7d1d7981017c (배포 후 동일 URL 갱신 예정).
Task 11 진행: Jin 결정 — Codex 릴리스 스레드 중단, +30으로 인수(내부 재업로드 O). 릴리스 브랜치 release/v2.0.8-30-20260901(05498001, pubspec 단일 커밋) → PR #249. Jin이 PLAY_INTERNAL_RELEASE_ENABLED=true 설정 완료(21:1x). PR CI green 대기 → squash 병합 예정.
Task 11 결과: PR #249 PR CI run 1028 success(Codex 리뷰 지적 0) → squash 병합 `1175c4f9`(제목 규약 그대로). main push CI **run 1029 success** — Analyze&Build success, `Signed AAB to Play Internal Testing` **success**(빌드→업로드 전 스텝). **내부테스트 트랙 업로드 완료, vC2215**.
Task 11 사고·수습: play_closed(비공개) 업로드 **실패** — `Version code 2215 has already been used.`(run 33563121115, 업로드 스텝만 실패). 원인은 구조적: versionCode=commit count이므로 같은 SHA로 두 트랙을 올리면 충돌하고, 변수 ON 상태에서는 main push마다 내부 업로드가 그 커밋의 versionCode를 먼저 소비한다. 과거 vC28 내부/vC29 비공개가 서로 다른 커밋이었던 이유가 이것(당시엔 우연히 회피). 같은 SHA에 두 번째 디스패치(run #6)가 겹쳐 있어 중복 업로드 실패를 피하려 내 디스패치를 취소하고 선행 run을 살렸음 — 선행 run의 발화 주체는 계정 액터가 동일해 특정 불가(Jin 또는 타 세션).
Task 11 complete: 양 트랙 업로드 종결 — 결과·근거는 `graphify-out/memory/2026-09-01-handoff-verification-and-release-record.md`가 정본(원장 중복 기재 회피, Codex Standards 지적 수용).
Ruling(컨트롤러): 게이트 문서를 사후 정당화로 고치지 않는다 — 재발 방지 규칙을 closed-testing-checklist-v2.md §2.2에 사실로 명문화하고, 비공개 재업로드는 변수 OFF 확인 후 새 main tip(commit count 2216+)에서 재디스패치한다. 대조 실측: 비공개가 이미 가진 vC2208(c82a7bef)과 1175c4f9의 차분은 ci.yml·문서·웹 lockfile·pubspec뿐 — 앱 바이너리 영향 0이므로 재업로드는 위생 목적이지 기능 전달이 아니다.
