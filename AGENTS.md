...
> **⛔ 정정(2026-08-30): 정물 카드 계열**(packs·activities·listening 의 800×600 카드
> 일러스트)도 위 'BIBLE 하나만' 문장이 stale하다. 이 계열의 우선순위는
> `docs/LISTENING_CARD_RECIPE.md`(생성 절차 정본) > `docs/assets/STYLE_LOCK.json`
> families `F-E-cards`(실측 불변값·픽셀 게이트 `tool/check_card_style.py`) > BIBLE 이다.
> BIBLE §1.2-3 '면 안에 그라데이션 없음'을 이 계열에 적용하면 평면 벡터가 나온다
> (2026-08-30 C1/C2 23장 전량 폐기 실측). 새 프롬프트를 쓰지 말고 RECIPE 고정
> 프롬프트에서 {SUBJECT}만 바꾼다.
...
- `lib/services/tts_service.dart` — **고품질 한국어 음성: 캐시우선 3단, 캐시 리비전 v3** (① 로컬캐시 `tts_v3_{voice}_{sha1}.mp3` → ② Firebase Storage `tts/v3/{voice}/{sha1}.mp3` 사전생성 → ③ Cloud Function 동적합성; OS 음성 폴백 없음). `speak(text,{voice})`/`speakSlow`/`setRate` 인터페이스 유지. voice: **female=Chirp3-HD-Zephyr**, **male=Chirp3-HD-Enceladus**. 정본 시나리오는 `playerCharacterId`와 실제 speaker ID를 캐릭터 프로필로 해석해 음성을 정하며, 캐릭터 ID가 없는 구형 장면에만 user=여 / 그 외=남 규칙을 폴백으로 쓴다. 나머지 기본 발화는 텍스트 SHA-1 기반 `auto` 정책으로 앱과 사전생성기가 같은 성별을 결정한다. **2026-09-22 corpus 12,630 (female 6,508 / male 6,122; `assets/data/tts_canonical_manifest.json`의 voices 배열 기준). 같은 날 `python tool/generate_tts.py --verify-storage` 읽기 검사: expected 12,630 / remote 21,143 / missing 0 / stale 8,513. 이는 키 완전성 검사이며 모든 클립의 사람 청취·실기기 재생이나 최신 함수 소스 배포를 뜻하지 않는다.** ⚠️ 콘텐츠 배치를 추가하면 이 숫자가 바로 낡는다 — 배치 직후 `python3 tool/generate_tts.py --verify-storage` 로 확인하고 누락이 있으면 `--missing-from-storage` 를 돌릴 것. 정본 후보는 `--scenario-pending-manifest`를 정확한 범위로 사용하고 `--verification-output` 영수증의 누락 0을 승격 전에 확인한다. 실제 음성명은 CF에만 있고 클라는 'female'/'male'만 전송한다. `audioplayers` 재생, sha1 키 계약은 `functions/tts/tts_contract.js` + `tool/generate_tts.py` + `test/tts_cache_key_test.dart` 3중 고정(동일 벡터). 버킷 `ko-lernen-app.firebasestorage.app`(europe-west3). 동적 CF `functions/tts/synthesize_tts`(callable, App Check enforce).
- **책 한 컷 (Phase 5)**:
...
- **단어팩 (Phase 1·2)**: `lib/services/vocab_pack_service.dart` (2026-09-22 `assets/data/korean_vocab.csv`의 2,944행·서로 다른 `pack_id` 252개 기준; 초안 팩·콘텐츠 표본 승인과 구분) + `pack_progress_service.dart` (진행도 로컬+Firestore `users/{uid}/packs`).
...
- ~~`tiger_stage_rive.dart`~~ — 위 항목과 함께 2026-08-06 폐지(Rive 리깅 폴백 체인 삭제). 현재 정본은 아래 `tiger_video.dart` 하나뿐이다.
