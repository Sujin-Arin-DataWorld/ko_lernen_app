...
- [x] 태블릿 레일은 긴 독일어 `Lerngruppe` 라벨과 시스템 글자 확대 1.3배에서도 안정적으로 렌더링되는 회귀 테스트를 갖췄다. 커밋 대기.
...
- Tablet rail accessibility: the long German `Lerngruppe` label is now covered at 1.3x system text scaling, alongside phone, portrait-tablet, wide-tablet, and selection behavior. Widget test and `git diff --check` pass. Commit: pending.
