...
- [ ] **UI 실기기 게이트 (Jin)**: 덱 4방향 손맛·시스템 엣지·히어로 잘림, 승인 대기 중인
  ...
  하지 않는다.
- [ ] **콘텐츠 UI 전역 개편 (계획, Jin §0 승인 대기)**: 정본
  `docs/CONTENT_UI_BIBLE.md`. 콘텐츠 CTA를 듣기 청금석(`info` `#57799E`)으로
  통일하고, 틴더 4방향·하단 4버튼을 인스타/틱톡 세로 피드+더블탭 하트(저장)로
  바꾸며, 듣기 플레이어/책장 분리·쓰기 규칙카드 퇴거·hero 카드 제거·좋아요
  전용 연습을 단계 P0–P7로 친다. Overhaul 2 §1-1·§1-2는 이 문서가 대체한다.
  구현은 Jin이 바이블 §0을 잠근 뒤에만. P0–P1(토큰·토스트)만 승인 전 버그수정
  으로 갈 수 있다.
