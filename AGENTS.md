...
- [x] UI/UX 개편 2 P1: 네 학습 덱의 카드 폭·플립 rect를 공통 슬롯으로 고정하고 덱 단위 타이포 측정과 회귀 센서를 추가했다. Flutter 3.44.0 analyze 0, 집중 배터리 29/29, 폭 핀 파괴-복원 RED→GREEN을 확인했다.
- [x] UI/UX 개편 2 P2: 4방향 스와이프, 덱 underlay, 미니 원형 action bar, 평가 없는 defer/save, 플립 우선 힌트와 공용 1회 코치를 네 덱에 배선했다. P2 핵심 46/46 및 화면별 SRS/wrong-count 무기록 센서를 통과했다.
- [x] UI/UX 개편 2 P3–P5: Today 활동 포스터·한옥/퀘스트/보자기 일러스트 언어, 카탈로그 4:3·ready footer 정리·Games hero, Gye showcase 압축·Hanok shortcut tile을 구현했다. 관련 매트릭스 460/460, Linux 골든 15/15와 직접 이미지 검수를 통과했다.
