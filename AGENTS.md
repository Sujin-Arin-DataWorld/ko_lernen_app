...
- [~] P2a: `site_base` + 전통 배치에 맞춘 6개 구조물의 7종 통일 지도 에셋을 제작 중이다. 첫 6개 레이어는 기계 투명도 검사에는 통과했지만 개별 3/4 소실점 때문에 하나의 전통 배치도로 읽히지 않는다는 사용자 피드백으로 반려했다. `site_base`·anchor·연못/다리 재사용 계약은 유지하고, **north-up plan-locked oblique**의 한 카메라로 여섯 채를 다시 제작한다. 기존 `gye_pond_large`·`gye_bridge`는 한 쌍으로 직접 재사용하고 흰 캔버스가 남은 기존 외부 장식은 정규화 전까지 차단한다.
