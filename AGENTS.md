...
- [x] **Codex 한옥 이식 3건 (파이프라인만)**: RGBa 리사이즈·원자 WebP 저장·맵
 위젯 provider 주입. lineage/stack 기본값·런타임 승격은 그대로. 상세는 세션 로그.
- [x] **TTS 대기/쿼터 + 단어망 Kurs**: 패자 합성 금지, 7s poll, quota는 OS TTS
 폴백 없음. 단어망은 코스 정답/완료 단원 vocab을 Gelernt에 합친다.
