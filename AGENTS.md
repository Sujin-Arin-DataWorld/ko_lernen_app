...
- [x] **살아 있는 한옥 — 배선·자동화·세분화 (2026-08-18, PR-B~D + Phase 2)**:
  `145e7928`로 **main에 병합됨**. 정본 계획은 Mac 로컬 `~/.claude/plans/swift-yawning-squirrel.md`
  (저장소 밖). PR-B: 원장 소급 기록(한옥 provenance 27레코드/48.0cr + 장식 ledger
  18레코드/65.0cr + discarded 28, 계획서의 44cr 추정은 틀렸었다) + 크레딧 상한 600.
  PR-C: `canDoSegmentEvidenceProgress()` 등 **데이터 절반만** — 실제 렌더 필터
  변경은 골든 테스트가 있는 라이브 위젯이라 PR-E cutover와 함께로 미뤘다. PR-D:
  방별 가구 풀 + A2 가구 12종이 모든 방에 노출되던 실버그 수정. Phase 2-1~2-3
  완성(STYLE_LOCK.json·check_style_conformance.py·ledger_append.py·asset_recipe.py).
  Phase 2-4는 `decoration_transparency_test.dart` 구멍만 폐쇄, 등록 자동화 러너는
  미착수. **PR-E(cutover)·PR-F(레벨별 발행)·Phase 3(별당·서고 신규 생성) 전부
  미착수.** 레거시 한옥이 정본인 상태는 변하지 않았다. 다음 한옥 작업은
  `chore/hanok-pr-e-prep`를 잇지 말고 `origin/main`에서 새 브랜치를 딴다.
- [ ] **사랑방 소품 감사 후속 (Jin 결정 대기)**: 참고 3장
  (`docs/assets/reference_images/`) F-A 실측 완료 — 백자술병 면분할 맞음/팔레트
  실패, 자개함·보석함은 워터컬러라 스타일 레퍼런스 금지. 출시 `decoration_jagae_mungap`은
  F-A 통과(함/궤 앵커). `chaekgado_prop_vase`도 F-A 실패. 재생성 vs 팔레트 예외 vs
  보류는 Jin이 정한다. 정본 인수인계:
