...
- [ ] **콘텐츠 UI 전역 개편 (계획, Jin §0 승인 대기)**: 정본
  `docs/CONTENT_UI_BIBLE.md`. 벤치마크(thevocabulary.app)의 뼈대만 가져온다:
  `?` 플립, 공유, 하트≠보관, 세로 피드. CTA는 듣기 청금석. 피부는 한지·면분할·
  외곽선 0. 공유 기본 이미지는 두루마리(안 A). Overhaul 2 §1-1·§1-2는 이 문서가
  대체한다. 구현은 Jin이 바이블 §0을 잠근 뒤에만. P0–P1만 승인 전 버그수정.
