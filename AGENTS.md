...
- `lib/services/tts_service.dart` — **고품질 한국어 음성: 캐시우선 3단, 캐시 리비전 v3** (① 로컬캐시 `tts_v3_{voice}_{sha1}.mp3` → ② Firebase Storage `tts/v3/{voice}/{sha1}.mp3` 사전생성 → ③ Cloud Function 동적합성; OS 음성 폴백 없음). `speak(text,{voice})`/`speakSlow`/`setRate` 인터페이스 유지. voice: **female=Chirp3-HD-Zephyr**, **male=Chirp3-HD-Enceladus**. 시나리오 대화는 화자별 voice (`scenario_player`: user=여 / 그 외=남)를 유지하고, 나머지 기본 발화는 텍스트 SHA-1 기반 `auto` 정책으로 앱과 사전생성기가 같은 성별을 결정한다. **2026-08-22 corpus 12,631 (female 6,381 / male 6,250), Storage expected 12,631, remote 17,662, missing 0, stale 5,031.** ⚠️ 콘텐츠 배치를 추가하면 이 숫자가 바로 낡는다 — 배치 직후 `python3 tool/generate_tts.py --verify-storage` 로 확인하고 누락이 있으면 `--missing-from-storage` 를 돌릴 것. 실제 음성명은 CF에만 있고 클라는 'female'/'male'만 전송한다. `audioplayers` 재생, sha1 키 계약은 `functions/tts/tts_contract.js` + `tool/generate_tts.py` + `test/tts_cache_key_test.dart` 3중 고정(동일 벡터). 버킷 `ko-lernen-app.firebasestorage.app`(europe-west3). 동적 CF `functions/tts/synthesize_tts`(callable, App Check enforce).
- **책 한 컷 (Phase 5)**:
