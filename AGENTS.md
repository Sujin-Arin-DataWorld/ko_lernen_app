...
- `lib/services/tts_service.dart` — **고품질 한국어 음성: 캐시우선 3단, 캐시 리비전 v3** (① 로컬캐시 `tts_v3_{voice}_{sha1}.mp3` → ② Firebase Storage `tts/v3/{voice}/{sha1}.mp3` 사전생성 → ③ Cloud Function 동적합성; OS 음성 폴백 없음). `speak(text,{voice})`/`speakSlow`/`setRate` 인터페이스 유지. voice: **female=Chirp3-HD-Zephyr**, **male=Chirp3-HD-Enceladus**. 정본 시나리오는 `playerCharacterId`와 실제 speaker ID를 캐릭터 프로필로 해석해 음성을 정하며, 캐릭터 ID가 없는 구형 장면에만 user=여 / 그 외=남 규칙을 폴백으로 쓴다. 나머지 기본 발화는 텍스트 SHA-1 기반 `auto` 정책으로 앱과 사전생성기가 같은 성별을 결정한다. **2026-09-15 corpus 11,642 (female 5,989 / male 5,653; via `python -c "import json,pathlib; v=json.loads(pathlib.Path('assets/data/tts_canonical_manifest.json').read_text(encoding='utf-8'))['voices']; print(len(v['female']), len(v['male']), len(v['female'])+len(v['male']))"`), Storage expected 11,642 — remote/missing/stale not re-verified in this pass; last Storage check (2026-08-22): remote 17,662, missing 0, stale 5,031. Re-run `python3 tool/generate_tts.py --verify-storage` to refresh against the new corpus size.** ⚠️ 콘텐츠 배치를 추가하면 이 숫자가 바로 낡는다 — 배치 직후 `python3 tool/generate_tts.py --verify-storage` 로 확인하고 누락이 있으면 `--missing-from-storage` 를 돌릴 것. 정본 후보는 `--scenario-pending-manifest`를 정확한 범위로 사용하고 `--verification-output` 영수증의 누락 0을 승격 전에 확인한다. 실제 음성명은 CF에만 있고 클라는 'female'/'male'만 전송한다. `audioplayers` 재생, sha1 키 계약은 `functions/tts/tts_contract.js` + `tool/generate_tts.py` + `test/tts_cache_key_test.dart` 3중 고정(동일 벡터). 버킷 `ko-lernen-app.firebasestorage.app`(europe-west3). 동적 CF `functions/tts/synthesize_tts`(callable, App Check enforce).
- **책 한 컷 (Phase 5)**:
