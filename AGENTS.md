...
- [~] P2a: `site_base` + 전통 배치에 맞춘 6개 구조물의 7종 통일 지도 에셋을 제작 중이다. `docs/superpowers/plans/2026-08-04-personal-hanok-master-map-p2a-assets.md`의 생성·투명도·합성 검수 계약을 따르며, 기존 `gye_pond_large`·`gye_bridge`는 한 쌍으로 직접 재사용하고 흰 캔버스가 남은 기존 외부 장식은 정규화 전까지 차단한다.
