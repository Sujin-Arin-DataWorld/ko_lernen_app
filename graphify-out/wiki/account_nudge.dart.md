# account_nudge.dart

> 14 nodes · cohesion 0.15

## Key Concepts

- **account_nudge.dart** (20 connections) — `lib/widgets/sori/account_nudge.dart`
- **../../services/account/account_transition_coordinator.dart** (5 connections)
- **../../services/account/account_ui_operations.dart** (5 connections)
- **../../services/auth_service.dart** (4 connections)
- **_AccountNudgeSheet** (3 connections) — `lib/widgets/sori/account_nudge.dart`
- **_AccountNudgeSheetState** (3 connections) — `lib/widgets/sori/account_nudge.dart`
- **account_operation_ui.dart** (1 connections)
- **accountOperations** (1 connections) — `lib/widgets/sori/account_nudge.dart`
- **build** (1 connections) — `lib/widgets/sori/account_nudge.dart`
- **_busy** (1 connections) — `lib/widgets/sori/account_nudge.dart`
- **_connectWith** (1 connections) — `lib/widgets/sori/account_nudge.dart`
- **createState** (1 connections) — `lib/widgets/sori/account_nudge.dart`
- **showAccountNudgeSheet** (1 connections) — `lib/widgets/sori/account_nudge.dart`
- **snapshot** (1 connections) — `lib/widgets/sori/account_nudge.dart`

## Relationships

- [String?](String_.md) (3 shared connections)
- [profile_screen.dart](profile_screen.dart.md) (3 shared connections)
- [_](_.md) (3 shared connections)
- [../../services/storage_service.dart](-..-services-storage_service.dart.md) (2 shared connections)
- [_State](_State.md) (2 shared connections)
- [ux_preview_app.dart](ux_preview_app.dart.md) (2 shared connections)
- [account_operation_ui.dart](account_operation_ui.dart.md) (2 shared connections)
- [account_link_failure_visibility_test.dart](account_link_failure_visibility_test.dart.md) (1 shared connections)
- [package:flutter/material.dart](package-flutter-material.dart.md) (1 shared connections)
- [main.dart](main.dart.md) (1 shared connections)

## Source Files

- `lib/widgets/sori/account_nudge.dart`

## Audit Trail

- EXTRACTED: 34 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*
