# game_reward.dart

> 44 nodes

## Key Concepts

- **game_reward.dart** (54 connections) — `lib/widgets/sori/game_reward.dart`
- **milestone_celebration.dart** (20 connections) — `lib/widgets/sori/milestone_celebration.dart`
- **ContentFeedbackContext** (9 connections) — `lib/models/content_feedback.dart`
- **GameOutcome** (7 connections) — `lib/widgets/sori/game_reward.dart`
- **mascot_preference.dart** (6 connections)
- **celebration.dart** (5 connections)
- **character_clip.dart** (4 connections)
- **ContentFeedbackSubmitter** (3 connections) — `lib/widgets/sori/content_feedback_sheet.dart`
- **content_feedback_card.dart** (2 connections)
- **content_feedback_sheet.dart** (2 connections)
- **../../data/milestone.dart** (2 connections)
- **_MilestoneBody** (2 connections) — `lib/widgets/sori/milestone_celebration.dart`
- **MascotKind? get** (2 connections)
- **int? score,
  bool** (1 connections)
- **actions** (1 connections) — `lib/widgets/sori/game_reward.dart`
- **best** (1 connections) — `lib/widgets/sori/game_reward.dart`
- **bestLabel** (1 connections) — `lib/widgets/sori/game_reward.dart`
- **build** (1 connections) — `lib/widgets/sori/game_reward.dart`
- **celebrate** (1 connections) — `lib/widgets/sori/game_reward.dart`
- **createState** (1 connections) — `lib/widgets/sori/game_reward.dart`
- **_ctrl** (1 connections) — `lib/widgets/sori/game_reward.dart`
- **dispose** (1 connections) — `lib/widgets/sori/game_reward.dart`
- **feedbackCompletedMissionIds** (1 connections) — `lib/widgets/sori/game_reward.dart`
- **feedbackContext** (1 connections) — `lib/widgets/sori/game_reward.dart`
- **feedbackFeatureGate** (1 connections) — `lib/widgets/sori/game_reward.dart`
- *... and 19 more nodes in this community*

## Relationships

- [String?](String_.md) (7 shared connections)
- [cloze_game_screen.dart](cloze_game_screen.dart.md) (6 shared connections)
- [content_feedback_card.dart](content_feedback_card.dart.md) (5 shared connections)
- [home_hero.dart](home_hero.dart.md) (2 shared connections)
- [path_trail.dart](path_trail.dart.md) (2 shared connections)
- [_](_.md) (2 shared connections)
- [vocab_pack_result_screen.dart](vocab_pack_result_screen.dart.md) (2 shared connections)
- [content_feedback_sheet.dart](content_feedback_sheet.dart.md) (2 shared connections)
- [content_feedback_lifecycle.dart](content_feedback_lifecycle.dart.md) (2 shared connections)
- [List](List.md) (2 shared connections)
- [_State](_State.md) (2 shared connections)
- [beta_mission_catalog.dart](beta_mission_catalog.dart.md) (2 shared connections)

## Source Files

- `lib/models/content_feedback.dart`
- `lib/widgets/sori/content_feedback_sheet.dart`
- `lib/widgets/sori/game_reward.dart`
- `lib/widgets/sori/milestone_celebration.dart`

## Audit Trail

- EXTRACTED: 102 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*