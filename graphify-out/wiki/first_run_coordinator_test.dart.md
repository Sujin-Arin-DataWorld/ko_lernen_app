# first_run_coordinator_test.dart

> 59 nodes

## Key Concepts

- **first_run_coordinator_test.dart** (80 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **LegacyOnboardingSnapshot** (3 connections) — `lib/features/onboarding_v2/first_run_coordinator.dart`
- **OnboardingPlacementHistoryConflictException** (2 connections) — `lib/features/onboarding_v2/first_run_coordinator.dart`
- **class _CommitGateway** (1 connections)
- **allowed** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **browse** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **browseWrites** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **calls** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **canonicalPlacement** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **canRecordOnboardingCompleted** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **canRecordOnboardingStarted** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **clear** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **companion** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **companionIdentityExplicitlyStored** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **companionVisible** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **companionWrites** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **completed** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **completionWrites** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **consent** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **coordinator** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **hasConsent** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **hasCourseHistory** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **implements** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **initializePlacement** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- **isLegacyOnboardingComplete** (1 connections) — `test/features/onboarding_v2/first_run_coordinator_test.dart`
- *... and 34 more nodes in this community*

## Relationships

- [app_flows_test.dart](app_flows_test.dart.md) (5 shared connections)
- [first_run_coordinator.dart](first_run_coordinator.dart.md) (4 shared connections)
- [app_shell_onboarding_analytics_retry_test.dart](app_shell_onboarding_analytics_retry_test.dart.md) (4 shared connections)
- [onboarding_journey_state.dart](onboarding_journey_state.dart.md) (2 shared connections)
- [package:flutter_test/flutter_test.dart](package-flutter_test-flutter_test.dart.md) (2 shared connections)
- [entry_onboarding_uiux_test.dart](entry_onboarding_uiux_test.dart.md) (1 shared connections)
- [bool get](bool_get.md) (1 shared connections)
- [cloud_sync_test.dart](cloud_sync_test.dart.md) (1 shared connections)
- [learning_semantic_contract.dart](learning_semantic_contract.dart.md) (1 shared connections)
- [List](List.md) (1 shared connections)
- [onboarding_app_adapters.dart](onboarding_app_adapters.dart.md) (1 shared connections)
- [dart:async](dart-async.md) (1 shared connections)

## Source Files

- `lib/features/onboarding_v2/first_run_coordinator.dart`
- `test/features/onboarding_v2/first_run_coordinator_test.dart`

## Audit Trail

- EXTRACTED: 83 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*