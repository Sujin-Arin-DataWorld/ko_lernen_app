# intro_gate_screen.dart

> 69 nodes

## Key Concepts

- **intro_gate_screen.dart** (98 connections) — `lib/screens/intro_gate_screen.dart`
- **_IntroSkipSurface** (3 connections) — `lib/screens/intro_gate_screen.dart`
- **_IntroSkipSurfaceState** (3 connections) — `lib/screens/intro_gate_screen.dart`
- **_IntroVideo** (3 connections) — `lib/screens/intro_gate_screen.dart`
- **_IntroVideoState** (3 connections) — `lib/screens/intro_gate_screen.dart`
- **_PendingGateSkip** (2 connections) — `lib/screens/intro_gate_screen.dart`
- **_Vignette** (2 connections) — `lib/screens/intro_gate_screen.dart`
- **OnboardingGateResult** (2 connections) — `lib/services/analytics_service.dart`
- **ActivateIntent** (1 connections)
- **_activate** (1 connections) — `lib/screens/intro_gate_screen.dart`
- **build** (1 connections) — `lib/screens/intro_gate_screen.dart`
- **child** (1 connections) — `lib/screens/intro_gate_screen.dart`
- **_codeStarted** (1 connections) — `lib/screens/intro_gate_screen.dart`
- **_complete** (1 connections) — `lib/screens/intro_gate_screen.dart`
- **_completeGateAndNavigate** (1 connections) — `lib/screens/intro_gate_screen.dart`
- **_courtyardAsset** (1 connections) — `lib/screens/intro_gate_screen.dart`
- **_coverGateFrame** (1 connections) — `lib/screens/intro_gate_screen.dart`
- **_coverImage** (1 connections) — `lib/screens/intro_gate_screen.dart`
- **createState** (1 connections) — `lib/screens/intro_gate_screen.dart`
- **_ctrl** (1 connections) — `lib/screens/intro_gate_screen.dart`
- **deferLeaseForTesting** (1 connections) — `lib/screens/intro_gate_screen.dart`
- **deferVideoLeaseForTesting** (1 connections) — `lib/screens/intro_gate_screen.dart`
- **didChangeDependencies** (1 connections) — `lib/screens/intro_gate_screen.dart`
- **dispose** (1 connections) — `lib/screens/intro_gate_screen.dart`
- **_done** (1 connections) — `lib/screens/intro_gate_screen.dart`
- *... and 44 more nodes in this community*

## Relationships

- [hanok_header.dart](hanok_header.dart.md) (4 shared connections)
- [onboarding_level_screen.dart](onboarding_level_screen.dart.md) (4 shared connections)
- [_State](_State.md) (4 shared connections)
- [StatelessWidget](StatelessWidget.md) (3 shared connections)
- [package:flutter/material.dart](package-flutter-material.dart.md) (3 shared connections)
- [gye_screen.dart](gye_screen.dart.md) (2 shared connections)
- [celebration.dart](celebration.dart.md) (2 shared connections)
- [app_shell.dart](app_shell.dart.md) (2 shared connections)
- [../../l10n/generated/app_localizations.dart](-..-l10n-generated-app_localizations.dart.md) (2 shared connections)
- [AnimationController](AnimationController.md) (1 shared connections)
- [speakable.dart](speakable.dart.md) (1 shared connections)
- [Map](Map.md) (1 shared connections)

## Source Files

- `lib/screens/intro_gate_screen.dart`
- `lib/services/analytics_service.dart`

## Audit Trail

- EXTRACTED: 107 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*