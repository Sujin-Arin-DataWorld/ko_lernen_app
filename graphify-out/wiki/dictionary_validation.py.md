# dictionary_validation.py

> 10 nodes

## Key Concepts

- **_exact_noun_in_response()** (5 connections) — `functions/analyze_korean_text/dictionary_validation.py`
- **validate_exact_noun()** (5 connections) — `functions/analyze_korean_text/dictionary_validation.py`
- **dictionary_validation.py** (5 connections) — `functions/analyze_korean_text/dictionary_validation.py`
- **test_dictionary_validation.py** (4 connections) — `functions/analyze_korean_text/test_dictionary_validation.py`
- **DictionaryValidationTest** (3 connections) — `functions/analyze_korean_text/test_dictionary_validation.py`
- **.test_accepts_only_an_exact_noun_headword()** (2 connections) — `functions/analyze_korean_text/test_dictionary_validation.py`
- **.test_rejects_a_matching_non_noun()** (2 connections) — `functions/analyze_korean_text/test_dictionary_validation.py`
- **Exact noun lookup against the Korean Basic Dictionary Open API. The game only…** (1 connections) — `functions/analyze_korean_text/dictionary_validation.py`
- **Returns True/False, or None when the protected API cannot be used.** (1 connections) — `functions/analyze_korean_text/dictionary_validation.py`
- **Pure checks for the Korean Basic Dictionary validity parser.** (1 connections) — `functions/analyze_korean_text/test_dictionary_validation.py`

## Relationships

- [main.py](main.py.md) (3 shared connections)

## Source Files

- `functions/analyze_korean_text/dictionary_validation.py`
- `functions/analyze_korean_text/test_dictionary_validation.py`

## Audit Trail

- EXTRACTED: 16 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*