# sori_stage_progression.dart

> 50 nodes · cohesion 0.04

## Key Concepts

- **sori_stage_progression.dart** (74 connections) — `lib/models/sori_stage_progression.dart`
- **../services/today_learning_snapshot.dart** (5 connections)
- **activityId** (1 connections) — `lib/models/sori_stage_progression.dart`
- **activityProgress** (1 connections) — `lib/models/sori_stage_progression.dart`
- **amount** (1 connections) — `lib/models/sori_stage_progression.dart`
- **arguments** (1 connections) — `lib/models/sori_stage_progression.dart`
- **colorRole** (1 connections) — `lib/models/sori_stage_progression.dart`
- **condition** (1 connections) — `lib/models/sori_stage_progression.dart`
- **current** (1 connections) — `lib/models/sori_stage_progression.dart`
- **de** (1 connections) — `lib/models/sori_stage_progression.dart`
- **description** (1 connections) — `lib/models/sori_stage_progression.dart`
- **detailRouteAliases** (1 connections) — `lib/models/sori_stage_progression.dart`
- **en** (1 connections) — `lib/models/sori_stage_progression.dart`
- **explanation** (1 connections) — `lib/models/sori_stage_progression.dart`
- **gameBests** (1 connections) — `lib/models/sori_stage_progression.dart`
- **gyeLanternCount** (1 connections) — `lib/models/sori_stage_progression.dart`
- **hanok** (1 connections) — `lib/models/sori_stage_progression.dart`
- **iconName** (1 connections) — `lib/models/sori_stage_progression.dart`
- **id** (1 connections) — `lib/models/sori_stage_progression.dart`
- **isActivityDescription** (1 connections) — `lib/models/sori_stage_progression.dart`
- **isEmpty** (1 connections) — `lib/models/sori_stage_progression.dart`
- **isUnlocked** (1 connections) — `lib/models/sori_stage_progression.dart`
- **items** (1 connections) — `lib/models/sori_stage_progression.dart`
- **key** (1 connections) — `lib/models/sori_stage_progression.dart`
- **kind** (1 connections) — `lib/models/sori_stage_progression.dart`
- *... and 25 more nodes in this community*

## Relationships

- [sori_stage_catalog_screen.dart](sori_stage_catalog_screen.dart.md) (4 shared connections)
- [@immutable](@immutable.md) (4 shared connections)
- [widgets/sori/tokens.dart](widgets-sori-tokens.dart.md) (2 shared connections)
- [bool get](bool_get.md) (1 shared connections)
- [int?](int_.md) (1 shared connections)
- [hanok_world_screen.dart](hanok_world_screen.dart.md) (1 shared connections)
- [gye_weekly_promise_navigation.dart](gye_weekly_promise_navigation.dart.md) (1 shared connections)
- [List](List.md) (1 shared connections)
- [Map](Map.md) (1 shared connections)
- [cloud_backup_deletion_test.dart](cloud_backup_deletion_test.dart.md) (1 shared connections)
- [String?](String_.md) (1 shared connections)
- [typedef](typedef.md) (1 shared connections)

## Source Files

- `lib/models/sori_stage_progression.dart`

## Audit Trail

- EXTRACTED: 78 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*
