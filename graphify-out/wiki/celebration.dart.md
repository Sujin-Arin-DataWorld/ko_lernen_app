# celebration.dart

> 35 nodes · cohesion 0.06

## Key Concepts

- **celebration.dart** (45 connections) — `lib/widgets/sori/celebration.dart`
- **_CelebrationLayerState** (4 connections) — `lib/widgets/sori/celebration.dart`
- **_CoinBurstLayerState** (4 connections) — `lib/widgets/sori/celebration.dart`
- **_CelebrationLayer** (3 connections) — `lib/widgets/sori/celebration.dart`
- **_CoinBurstLayer** (3 connections) — `lib/widgets/sori/celebration.dart`
- **angle** (1 connections) — `lib/widgets/sori/celebration.dart`
- **asset** (1 connections) — `lib/widgets/sori/celebration.dart`
- **_assets** (1 connections) — `lib/widgets/sori/celebration.dart`
- **build** (1 connections) — `lib/widgets/sori/celebration.dart`
- **burst** (1 connections) — `lib/widgets/sori/celebration.dart`
- **_Coin** (1 connections) — `lib/widgets/sori/celebration.dart`
- **_coins** (1 connections) — `lib/widgets/sori/celebration.dart`
- **color** (1 connections) — `lib/widgets/sori/celebration.dart`
- **count** (1 connections) — `lib/widgets/sori/celebration.dart`
- **createState** (1 connections) — `lib/widgets/sori/celebration.dart`
- **_ctrl** (1 connections) — `lib/widgets/sori/celebration.dart`
- **delay** (1 connections) — `lib/widgets/sori/celebration.dart`
- **_diamond** (1 connections) — `lib/widgets/sori/celebration.dart`
- **dispose** (1 connections) — `lib/widgets/sori/celebration.dart`
- **_gravity** (1 connections) — `lib/widgets/sori/celebration.dart`
- **initState** (1 connections) — `lib/widgets/sori/celebration.dart`
- **onDone** (1 connections) — `lib/widgets/sori/celebration.dart`
- **origin** (1 connections) — `lib/widgets/sori/celebration.dart`
- **paint** (1 connections) — `lib/widgets/sori/celebration.dart`
- **_palette** (1 connections) — `lib/widgets/sori/celebration.dart`
- *... and 10 more nodes in this community*

## Relationships

- [_State](_State.md) (6 shared connections)
- [String?](String_.md) (4 shared connections)
- [hanok_cinematic.dart](hanok_cinematic.dart.md) (1 shared connections)
- [List](List.md) (1 shared connections)
- [standalone_games_uiux_test.dart](standalone_games_uiux_test.dart.md) (1 shared connections)
- [static const](static_const.md) (1 shared connections)
- [CustomPainter](CustomPainter.md) (1 shared connections)
- [dart:math](dart-math.md) (1 shared connections)
- [package:flutter/material.dart](package-flutter-material.dart.md) (1 shared connections)

## Source Files

- `lib/widgets/sori/celebration.dart`

## Audit Trail

- EXTRACTED: 53 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*
