# book_capture_screen.dart

> 38 nodes · cohesion 0.06

## Key Concepts

- **book_capture_screen.dart** (54 connections) — `lib/screens/book_capture_screen.dart`
- **../services/book_image_service.dart** (6 connections)
- **BookCaptureScreen** (3 connections) — `lib/screens/book_capture_screen.dart`
- **_BookCaptureScreenState** (3 connections) — `lib/screens/book_capture_screen.dart`
- **_ErrorCard** (2 connections) — `lib/screens/book_capture_screen.dart`
- **_pick** (2 connections) — `lib/screens/book_capture_screen.dart`
- **_resumeRecoveredBook** (2 connections) — `lib/screens/book_capture_screen.dart`
- **../services/crop_recovery_service.dart** (2 connections)
- **../services/picker_recovery_service.dart** (2 connections)
- **../widgets/sori/feature_coach.dart** (2 connections)
- **Route /book/preview** (2 connections)
- **bookCaptureJpegQuality** (1 connections) — `lib/screens/book_capture_screen.dart`
- **bookCaptureQuotaBlocksPick** (1 connections) — `lib/screens/book_capture_screen.dart`
- **build** (1 connections) — `lib/screens/book_capture_screen.dart`
- **buildBookPreviewArguments** (1 connections) — `lib/screens/book_capture_screen.dart`
- **_busy** (1 connections) — `lib/screens/book_capture_screen.dart`
- **captureMode** (1 connections) — `lib/screens/book_capture_screen.dart`
- **_clearRecoveredAfterHandoff** (1 connections) — `lib/screens/book_capture_screen.dart`
- **createState** (1 connections) — `lib/screens/book_capture_screen.dart`
- **_cropSequence** (1 connections) — `lib/screens/book_capture_screen.dart`
- **_cropSession** (1 connections) — `lib/screens/book_capture_screen.dart`
- **_discardUnlessDurablyRecovered** (1 connections) — `lib/screens/book_capture_screen.dart`
- **_errorKey** (1 connections) — `lib/screens/book_capture_screen.dart`
- **existingPackId** (1 connections) — `lib/screens/book_capture_screen.dart`
- **initState** (1 connections) — `lib/screens/book_capture_screen.dart`
- *... and 13 more nodes in this community*

## Relationships

- [widgets/sori/tokens.dart](widgets-sori-tokens.dart.md) (5 shared connections)
- [main.dart](main.dart.md) (3 shared connections)
- [String?](String_.md) (2 shared connections)
- [word_image_service.dart](word_image_service.dart.md) (2 shared connections)
- [gye_screen.dart](gye_screen.dart.md) (2 shared connections)
- [_State](_State.md) (2 shared connections)
- [return](return.md) (1 shared connections)
- [package:flutter/foundation.dart](package-flutter-foundation.dart.md) (1 shared connections)
- [package:flutter/material.dart](package-flutter-material.dart.md) (1 shared connections)
- [crop_recovery_service.dart](crop_recovery_service.dart.md) (1 shared connections)
- [../../services/storage_service.dart](-..-services-storage_service.dart.md) (1 shared connections)
- [profile_screen.dart](profile_screen.dart.md) (1 shared connections)

## Source Files

- `lib/screens/book_capture_screen.dart`

## Audit Trail

- EXTRACTED: 68 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*
