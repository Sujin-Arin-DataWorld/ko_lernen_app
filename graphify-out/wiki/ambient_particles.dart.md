# ambient_particles.dart

> 31 nodes

## Key Concepts

- **ambient_particles.dart** (37 connections) — `lib/widgets/sori/ambient_particles.dart`
- **_AmbientParticlesState** (4 connections) — `lib/widgets/sori/ambient_particles.dart`
- **AmbientParticles** (3 connections) — `lib/widgets/sori/ambient_particles.dart`
- **Brightness** (2 connections)
- **brightness** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **build** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **_c** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **colorIdx** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **count** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **createState** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **cycles** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **dispose** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **_drawPetal** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **initState** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **isDark** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **_moteColors** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **offset** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **opacity** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **paint** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **_Particle** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **_particles** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **_petalColors** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **rotCycles** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **rotDir** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- **shouldRepaint** (1 connections) — `lib/widgets/sori/ambient_particles.dart`
- *... and 6 more nodes in this community*

## Relationships

- [package:flutter/material.dart](package-flutter-material.dart.md) (2 shared connections)
- [_State](_State.md) (2 shared connections)
- [_](_.md) (1 shared connections)
- [AnimationController](AnimationController.md) (1 shared connections)
- [List](List.md) (1 shared connections)
- [static const](static_const.md) (1 shared connections)
- [CustomPainter](CustomPainter.md) (1 shared connections)
- [package:flutter/services.dart](package-flutter-services.dart.md) (1 shared connections)
- [celebration.dart](celebration.dart.md) (1 shared connections)

## Source Files

- `lib/widgets/sori/ambient_particles.dart`

## Audit Trail

- EXTRACTED: 42 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*