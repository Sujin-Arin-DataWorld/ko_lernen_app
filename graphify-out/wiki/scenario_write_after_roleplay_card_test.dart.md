# scenario_write_after_roleplay_card_test.dart

> 20 nodes · cohesion 0.10

## Key Concepts

- **scenario_write_after_roleplay_card_test.dart** (30 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **KoreanProofreadingResult** (4 connections) — `lib/services/korean_proofreading_service.dart`
- **app** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **availability** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **check** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **close** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **download** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **downloadAvailability** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **downloadCalls** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **ensureVisible** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **_evidence** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **_evidenceWithoutGrammar** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **finder** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **main** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **proofread** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **proofreadCalls** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **pump** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **result** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **tap** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`
- **_tapVisible** (1 connections) — `test/scenario_write_after_roleplay_card_test.dart`

## Relationships

- [scenario_writing_check_service_test.dart](scenario_writing_check_service_test.dart.md) (4 shared connections)
- [package:ko_lernen_app/l10n/generated/app_localizations.dart](package-ko_lernen_app-l10n-generated-app_localizations.dart.md) (2 shared connections)
- [@immutable](@immutable.md) (1 shared connections)
- [korean_proofreading_service.dart](korean_proofreading_service.dart.md) (1 shared connections)
- [custom_pack_games_uiux_test.dart](custom_pack_games_uiux_test.dart.md) (1 shared connections)
- [scenario_writing_check_service.dart](scenario_writing_check_service.dart.md) (1 shared connections)
- [package:flutter/material.dart](package-flutter-material.dart.md) (1 shared connections)
- [package:flutter_test/flutter_test.dart](package-flutter_test-flutter_test.dart.md) (1 shared connections)
- [test/profile_screen_test.dart](test-profile_screen_test.dart.md) (1 shared connections)
- [quest_engines_uiux_test.dart](quest_engines_uiux_test.dart.md) (1 shared connections)

## Source Files

- `lib/services/korean_proofreading_service.dart`
- `test/scenario_write_after_roleplay_card_test.dart`

## Audit Trail

- EXTRACTED: 33 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*
