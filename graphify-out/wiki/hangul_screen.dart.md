# hangul_screen.dart

> 129 nodes · cohesion 0.02

## Key Concepts

- **hangul_screen.dart** (150 connections) — `lib/screens/hangul_screen.dart`
- **daily_char_sheet.dart** (59 connections) — `lib/screens/daily_char_sheet.dart`
- **_Timer** (14 connections) — `lib/screens/kkeunmari_screen.dart`
- **../widgets/sori/content_feedback_card.dart** (14 connections)
- **daily_char_service.dart** (4 connections) — `lib/services/daily_char_service.dart`
- **../data/hangul_data.dart** (3 connections)
- **DailyCalligraphyRouteScreen** (2 connections) — `lib/screens/daily_char_sheet.dart`
- **_CharCell** (2 connections) — `lib/screens/hangul_screen.dart`
- **_CharGrid** (2 connections) — `lib/screens/hangul_screen.dart`
- **_DetailSheet** (2 connections) — `lib/screens/hangul_screen.dart`
- **_HangulCardFace** (2 connections) — `lib/screens/hangul_screen.dart`
- **_HangulCardsViewport** (2 connections) — `lib/screens/hangul_screen.dart`
- **_OverviewTab** (2 connections) — `lib/screens/hangul_screen.dart`
- **_SectionLabel** (2 connections) — `lib/screens/hangul_screen.dart`
- **_SyllableDemo** (2 connections) — `lib/screens/hangul_screen.dart`
- **../services/stroke_matcher.dart** (2 connections)
- **_FakeTimer** (2 connections) — `test/vocab_pack_advance_timer_test.dart`
- **../widgets/stroke_canvas.dart** (2 connections)
- **../widgets/trace_canvas.dart** (2 connections)
- **HangulChar get** (1 connections)
- **_acceptedStrokes** (1 connections) — `lib/screens/daily_char_sheet.dart`
- **_acceptStroke** (1 connections) — `lib/screens/daily_char_sheet.dart`
- **_beginTracing** (1 connections) — `lib/screens/daily_char_sheet.dart`
- **build** (1 connections) — `lib/screens/daily_char_sheet.dart`
- **_canFinish** (1 connections) — `lib/screens/daily_char_sheet.dart`
- *... and 104 more nodes in this community*

## Relationships

- [StatelessWidget](StatelessWidget.md) (11 shared connections)
- [widgets/sori/tokens.dart](widgets-sori-tokens.dart.md) (9 shared connections)
- [_State](_State.md) (8 shared connections)
- [package:flutter/services.dart](package-flutter-services.dart.md) (8 shared connections)
- [cloze_game_screen.dart](cloze_game_screen.dart.md) (7 shared connections)
- [legacy_vocab_screen.dart](legacy_vocab_screen.dart.md) (6 shared connections)
- [book_result_screen.dart](book_result_screen.dart.md) (4 shared connections)
- [listening_play_screen.dart](listening_play_screen.dart.md) (4 shared connections)
- [String?](String_.md) (3 shared connections)
- [sori_stage_today_screen.dart](sori_stage_today_screen.dart.md) (3 shared connections)
- [bool get](bool_get.md) (2 shared connections)
- [List](List.md) (2 shared connections)

## Source Files

- `lib/screens/daily_char_sheet.dart`
- `lib/screens/hangul_screen.dart`
- `lib/screens/kkeunmari_screen.dart`
- `lib/services/daily_char_service.dart`
- `test/vocab_pack_advance_timer_test.dart`

## Audit Trail

- EXTRACTED: 247 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*