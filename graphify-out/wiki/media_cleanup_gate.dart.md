# media_cleanup_gate.dart

> 24 nodes

## Key Concepts

- **media_cleanup_gate.dart** (22 connections) — `lib/services/account/media_cleanup_gate.dart`
- **CloudWriteSessionController** (13 connections) — `lib/services/account/cloud_write_session.dart`
- **cloud_write_session.dart** (10 connections)
- **cloud_restore_result.dart** (6 connections) — `lib/services/account/cloud_restore_result.dart`
- **CloudWriteResult** (5 connections) — `lib/services/account/cloud_write_session.dart`
- **account_transition_journal.dart** (4 connections)
- **CloudRestoreComponentResult** (2 connections) — `lib/services/account/cloud_restore_result.dart`
- **AccountTransitionJournalReader** (2 connections) — `lib/services/account/media_cleanup_gate.dart`
- **CloudRestoreResult** (1 connections) — `lib/services/account/cloud_restore_result.dart`
- **hasRemoteData** (1 connections) — `lib/services/account/cloud_restore_result.dart`
- **status** (1 connections) — `lib/services/account/cloud_restore_result.dart`
- **authorization** (1 connections) — `lib/services/account/media_cleanup_gate.dart`
- **_AuthorizationResult** (1 connections) — `lib/services/account/media_cleanup_gate.dart`
- **_authorize** (1 connections) — `lib/services/account/media_cleanup_gate.dart`
- **call** (1 connections) — `lib/services/account/media_cleanup_gate.dart`
- **_isCurrent** (1 connections) — `lib/services/account/media_cleanup_gate.dart`
- **key** (1 connections) — `lib/services/account/media_cleanup_gate.dart`
- **_matchExpected** (1 connections) — `lib/services/account/media_cleanup_gate.dart`
- **_MediaCleanupAuthorization** (1 connections) — `lib/services/account/media_cleanup_gate.dart`
- **MediaCleanupGate** (1 connections) — `lib/services/account/media_cleanup_gate.dart`
- **result** (1 connections) — `lib/services/account/media_cleanup_gate.dart`
- **run** (1 connections) — `lib/services/account/media_cleanup_gate.dart`
- **sessions** (1 connections) — `lib/services/account/media_cleanup_gate.dart`
- **SharedPreferencesAccountTransitionJournalReader** (1 connections) — `lib/services/account/media_cleanup_gate.dart`

## Relationships

- [_](_.md) (13 shared connections)
- [dart:async](dart-async.md) (3 shared connections)
- [account_operation_client.dart](account_operation_client.dart.md) (2 shared connections)
- [first_link_backfill.dart](first_link_backfill.dart.md) (2 shared connections)
- [cloud_write_session.dart](cloud_write_session.dart.md) (2 shared connections)
- [package:flutter_test/flutter_test.dart](package-flutter_test-flutter_test.dart.md) (2 shared connections)
- [account_transition_journal.dart](account_transition_journal.dart.md) (1 shared connections)
- [bookshelf_sync_outbox.dart](bookshelf_sync_outbox.dart.md) (1 shared connections)
- [cloud_sync_test.dart](cloud_sync_test.dart.md) (1 shared connections)
- [premium_service.dart](premium_service.dart.md) (1 shared connections)
- [push_service.dart](push_service.dart.md) (1 shared connections)
- [account_deletion_receipt_recovery_test.dart](account_deletion_receipt_recovery_test.dart.md) (1 shared connections)

## Source Files

- `lib/services/account/cloud_restore_result.dart`
- `lib/services/account/cloud_write_session.dart`
- `lib/services/account/media_cleanup_gate.dart`

## Audit Trail

- EXTRACTED: 56 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*