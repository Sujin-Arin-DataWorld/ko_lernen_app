# curriculum_evidence_projector.dart

> 8 nodes · cohesion 0.25

## Key Concepts

- **curriculum_evidence_projector.dart** (8 connections) — `lib/features/onboarding_v2/curriculum_evidence_projector.dart`
- **../../models/curriculum_alignment_contract.dart** (3 connections)
- **OnboardingCurriculumEvidenceProjection** (2 connections) — `lib/features/onboarding_v2/curriculum_evidence_projector.dart`
- **../../data/curriculum_alignment_registry.dart** (1 connections)
- **OnboardingCurriculumEvidenceProjector** (1 connections) — `lib/features/onboarding_v2/curriculum_evidence_projector.dart`
- **project** (1 connections) — `lib/features/onboarding_v2/curriculum_evidence_projector.dart`
- **_referenceIdentity** (1 connections) — `lib/features/onboarding_v2/curriculum_evidence_projector.dart`
- **references** (1 connections) — `lib/features/onboarding_v2/curriculum_evidence_projector.dart`

## Relationships

- [StatelessWidget](StatelessWidget.md) (2 shared connections)
- [List](List.md) (1 shared connections)
- [curriculum_alignment_registry.dart](curriculum_alignment_registry.dart.md) (1 shared connections)

## Source Files

- `lib/features/onboarding_v2/curriculum_evidence_projector.dart`

## Audit Trail

- EXTRACTED: 11 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*
