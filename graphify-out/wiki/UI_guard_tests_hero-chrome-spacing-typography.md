# UI guard tests (hero/chrome/spacing/typography)

> 14 nodes · cohesion 0.15

## Key Concepts

- **UI guard tests (hero/chrome/spacing/typography)** (5 connections) — `docs/CONTENT_UI_BIBLE.md`
- **SoriIllustratedCard 16:10 safe area** (4 connections) — `docs/LISTENING_CARD_ART_SPEC.md`
- **ChaekgadoShelfCase** (3 connections) — `docs/LISTENING_CARD_ART_SPEC.md`
- **ChaekgadoShelfCase safe area** (3 connections) — `docs/LISTENING_CARD_ART_SPEC.md`
- **One screen contract** (2 connections) — `docs/CONTENT_UI_BIBLE.md`
- **SoriGaps spacing rhythm grammar** (2 connections) — `docs/CONTENT_UI_BIBLE.md`
- **SoriLayout hero budget (heroMaxShare 0.22)** (2 connections) — `docs/CONTENT_UI_BIBLE.md`
- **Six-point inspection checklist** (2 connections) — `docs/LISTENING_CARD_RECIPE.md`
- **Bible governance verdict log** (1 connections) — `docs/CONTENT_UI_BIBLE.md`
- **Share image Plan A (hanji scroll)** (1 connections) — `docs/CONTENT_UI_BIBLE.md`
- **SoriChromeRow** (1 connections) — `docs/CONTENT_UI_BIBLE.md`
- **Spacing tokens (4dp base, 8dp grid)** (1 connections) — `docs/HANGUL_SORI_DESIGN_TOKENS.md`
- **Chaekgado shelf UI re-adoption** (1 connections) — `docs/LISTENING_CARD_ART_SPEC.md`
- **SoriIllustratedCard** (1 connections) — `docs/LISTENING_CARD_ART_SPEC.md`

## Relationships

- [실행 순서](실행_순서.md) (1 shared connections)

## Source Files

- `docs/CONTENT_UI_BIBLE.md`
- `docs/HANGUL_SORI_DESIGN_TOKENS.md`
- `docs/LISTENING_CARD_ART_SPEC.md`
- `docs/LISTENING_CARD_RECIPE.md`

## Audit Trail

- EXTRACTED: 12 (80%)
- INFERRED: 3 (20%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*