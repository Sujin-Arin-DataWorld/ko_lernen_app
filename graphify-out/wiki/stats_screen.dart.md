# stats_screen.dart

> 40 nodes · cohesion 0.05

## Key Concepts

- **stats_screen.dart** (59 connections) — `lib/screens/stats_screen.dart`
- **_StatsScreenState** (4 connections) — `lib/screens/stats_screen.dart`
- **StatsScreen** (3 connections) — `lib/screens/stats_screen.dart`
- **build** (2 connections) — `lib/screens/stats_screen.dart`
- **_MetricRow** (2 connections) — `lib/screens/stats_screen.dart`
- **_StatCard** (2 connections) — `lib/screens/stats_screen.dart`
- **_StreakHero** (2 connections) — `lib/screens/stats_screen.dart`
- **_StreakWeekHeatmap** (2 connections) — `lib/screens/stats_screen.dart`
- **_XpCard** (2 connections) — `lib/screens/stats_screen.dart`
- **accent** (1 connections) — `lib/screens/stats_screen.dart`
- **_badgeMeta** (1 connections) — `lib/screens/stats_screen.dart`
- **badges** (1 connections) — `lib/screens/stats_screen.dart`
- **badgesTitle** (1 connections) — `lib/screens/stats_screen.dart`
- **best** (1 connections) — `lib/screens/stats_screen.dart`
- **bestLabel** (1 connections) — `lib/screens/stats_screen.dart`
- **buildCoachSteps** (1 connections) — `lib/screens/stats_screen.dart`
- **coachId** (1 connections) — `lib/screens/stats_screen.dart`
- **color** (1 connections) — `lib/screens/stats_screen.dart`
- **createState** (1 connections) — `lib/screens/stats_screen.dart`
- **icon** (1 connections) — `lib/screens/stats_screen.dart`
- **initState** (1 connections) — `lib/screens/stats_screen.dart`
- **label** (1 connections) — `lib/screens/stats_screen.dart`
- **level** (1 connections) — `lib/screens/stats_screen.dart`
- **noBadges** (1 connections) — `lib/screens/stats_screen.dart`
- **now** (1 connections) — `lib/screens/stats_screen.dart`
- *... and 15 more nodes in this community*

## Relationships

- [widgets/sori/tokens.dart](widgets-sori-tokens.dart.md) (6 shared connections)
- [StatelessWidget](StatelessWidget.md) (5 shared connections)
- [_State](_State.md) (3 shared connections)
- [String?](String_.md) (2 shared connections)
- [listening_play_screen.dart](listening_play_screen.dart.md) (2 shared connections)
- [../../l10n/generated/app_localizations.dart](-..-l10n-generated-app_localizations.dart.md) (2 shared connections)
- [package:flutter/services.dart](package-flutter-services.dart.md) (2 shared connections)
- [wordbook_add.dart](wordbook_add.dart.md) (1 shared connections)
- [List](List.md) (1 shared connections)
- [static const](static_const.md) (1 shared connections)
- [package:flutter/material.dart](package-flutter-material.dart.md) (1 shared connections)
- [cloze_game_screen.dart](cloze_game_screen.dart.md) (1 shared connections)

## Source Files

- `lib/screens/stats_screen.dart`

## Audit Trail

- EXTRACTED: 69 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*