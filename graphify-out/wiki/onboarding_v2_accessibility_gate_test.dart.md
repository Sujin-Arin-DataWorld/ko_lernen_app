# onboarding_v2_accessibility_gate_test.dart

> 26 nodes

## Key Concepts

- **onboarding_v2_accessibility_gate_test.dart** (49 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **package:ko_lernen_app/screens/study_library_screen.dart** (2 connections)
- **bookmarkRecord** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **data** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **_expect48DpButton** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **_expectHeader** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **_expectInsideSafeViewport** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **_expectLabeled48DpButton** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **_germanApp** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **likedRecord** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **_localizedApp** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **main** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **_memoryBookmarkStore** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **_phoneSafeInsets** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **raw** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **readBookmarks** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **readBookshelfItems** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **readCustomPackItems** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **readLiked** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **readSrsRecords** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **records** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **rect** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **_setCompactViewport** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **_setViewport** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- **size** (1 connections) — `test/onboarding_v2_accessibility_gate_test.dart`
- *... and 1 more nodes in this community*

## Relationships

- [guide_ui_test.dart](guide_ui_test.dart.md) (4 shared connections)
- [app_flows_test.dart](app_flows_test.dart.md) (3 shared connections)
- [package:shared_preferences/shared_preferences.dart](package-shared_preferences-shared_preferences.dart.md) (2 shared connections)
- [onboarding_v2_presentation_test.dart](onboarding_v2_presentation_test.dart.md) (2 shared connections)
- [study_library_screen_test.dart](study_library_screen_test.dart.md) (1 shared connections)
- [a1_hanok_construction_map_test.dart](a1_hanok_construction_map_test.dart.md) (1 shared connections)
- [study_activity_responsive_test.dart](study_activity_responsive_test.dart.md) (1 shared connections)
- [List](List.md) (1 shared connections)
- [StudyLibraryBookmarkReader](StudyLibraryBookmarkReader.md) (1 shared connections)
- [study_library_repository_test.dart](study_library_repository_test.dart.md) (1 shared connections)
- [StudyLibraryCustomPackReader](StudyLibraryCustomPackReader.md) (1 shared connections)
- [StudyLibraryLikedReader](StudyLibraryLikedReader.md) (1 shared connections)

## Source Files

- `test/onboarding_v2_accessibility_gate_test.dart`

## Audit Trail

- EXTRACTED: 50 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*