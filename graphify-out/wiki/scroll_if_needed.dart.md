# scroll_if_needed.dart

> 6 nodes · cohesion 0.40

## Key Concepts

- **scroll_if_needed.dart** (6 connections) — `lib/widgets/sori/scroll_if_needed.dart`
- **ScrollPhysics?** (3 connections)
- **ScrollOnlyIfOverflowing** (2 connections) — `lib/widgets/sori/scroll_if_needed.dart`
- **applyTo** (1 connections) — `lib/widgets/sori/scroll_if_needed.dart`
- **kSoriCardFacePhysics** (1 connections) — `lib/widgets/sori/scroll_if_needed.dart`
- **shouldAcceptUserOffset** (1 connections) — `lib/widgets/sori/scroll_if_needed.dart`

## Relationships

- [package:flutter/material.dart](package-flutter-material.dart.md) (1 shared connections)
- [String?](String_.md) (1 shared connections)

## Source Files

- `lib/widgets/sori/scroll_if_needed.dart`

## Audit Trail

- EXTRACTED: 8 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*
