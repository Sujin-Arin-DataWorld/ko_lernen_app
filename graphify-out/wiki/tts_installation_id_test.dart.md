# tts_installation_id_test.dart

> 18 nodes

## Key Concepts

- **tts_installation_id_test.dart** (19 connections) — `test/tts_installation_id_test.dart`
- **TtsInstallationIdStore** (4 connections) — `lib/services/tts_installation_id.dart`
- **SecureTtsInstallationIdStore** (2 connections) — `lib/services/tts_installation_id.dart`
- **_DelayedStore** (2 connections) — `test/tts_installation_id_test.dart`
- **_MemoryStore** (2 connections) — `test/tts_installation_id_test.dart`
- **package:ko_lernen_app/services/tts_installation_id.dart** (1 connections)
- **failRead** (1 connections) — `test/tts_installation_id_test.dart`
- **failWrite** (1 connections) — `test/tts_installation_id_test.dart`
- **_firstId** (1 connections) — `test/tts_installation_id_test.dart`
- **main** (1 connections) — `test/tts_installation_id_test.dart`
- **read** (1 connections) — `test/tts_installation_id_test.dart`
- **reads** (1 connections) — `test/tts_installation_id_test.dart`
- **readStarted** (1 connections) — `test/tts_installation_id_test.dart`
- **releaseRead** (1 connections) — `test/tts_installation_id_test.dart`
- **_secondId** (1 connections) — `test/tts_installation_id_test.dart`
- **value** (1 connections) — `test/tts_installation_id_test.dart`
- **write** (1 connections) — `test/tts_installation_id_test.dart`
- **writes** (1 connections) — `test/tts_installation_id_test.dart`

## Relationships

- [tts_installation_id.dart](tts_installation_id.dart.md) (2 shared connections)
- [cloud_sync_test.dart](cloud_sync_test.dart.md) (1 shared connections)
- [String?](String_.md) (1 shared connections)
- [dart:async](dart-async.md) (1 shared connections)
- [package:flutter_test/flutter_test.dart](package-flutter_test-flutter_test.dart.md) (1 shared connections)

## Source Files

- `lib/services/tts_installation_id.dart`
- `test/tts_installation_id_test.dart`

## Audit Trail

- EXTRACTED: 24 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*