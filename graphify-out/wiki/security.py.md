# security.py

> 60 nodes · cohesion 0.08

## Key Concepts

- **security.py** (37 connections) — `functions/analyze_korean_text/security.py`
- **test_security.py** (27 connections) — `functions/analyze_korean_text/test_security.py`
- **Any** (20 connections)
- **FirestoreIdempotencyGate** (17 connections) — `functions/analyze_korean_text/security.py`
- **QuotaState** (14 connections) — `functions/analyze_korean_text/security.py`
- **datetime** (13 connections)
- **consume_quota_state()** (12 connections) — `functions/analyze_korean_text/security.py`
- **FirestoreQuotaGate** (11 connections) — `functions/analyze_korean_text/security.py`
- **QuotaPolicyTest** (10 connections) — `functions/analyze_korean_text/test_security.py`
- **idempotency_payload()** (9 connections) — `functions/analyze_korean_text/security.py`
- **is_current_idempotency()** (8 connections) — `functions/analyze_korean_text/security.py`
- **release_quota_state()** (8 connections) — `functions/analyze_korean_text/security.py`
- **._client()** (7 connections) — `functions/analyze_korean_text/security.py`
- **._reference()** (7 connections) — `functions/analyze_korean_text/security.py`
- **.consume()** (7 connections) — `functions/analyze_korean_text/security.py`
- **run_with_deadline()** (7 connections) — `functions/analyze_korean_text/security.py`
- **._claim_once()** (6 connections) — `functions/analyze_korean_text/security.py`
- **._ledger_reference()** (6 connections) — `functions/analyze_korean_text/security.py`
- **.release()** (6 connections) — `functions/analyze_korean_text/security.py`
- **quota_expires_at()** (6 connections) — `functions/analyze_korean_text/security.py`
- **.claim()** (5 connections) — `functions/analyze_korean_text/security.py`
- **._payload()** (5 connections) — `functions/analyze_korean_text/security.py`
- **QuotaPolicy** (4 connections) — `functions/analyze_korean_text/security.py`
- **_as_utc_datetime()** (4 connections) — `functions/analyze_korean_text/security.py`
- **_firebase_app()** (4 connections) — `functions/analyze_korean_text/security.py`
- *... and 35 more nodes in this community*

## Relationships

- [IdempotencyPolicyTest](IdempotencyPolicyTest.md) (13 shared connections)
- [main.py](main.py.md) (12 shared connections)
- [EndpointSecurityTest](EndpointSecurityTest.md) (12 shared connections)
- [verify_caller](verify_caller.md) (10 shared connections)
- [CircuitBreaker](CircuitBreaker.md) (3 shared connections)
- [validate_directory](validate_directory.md) (1 shared connections)

## Source Files

- `functions/analyze_korean_text/security.py`
- `functions/analyze_korean_text/test_security.py`

## Audit Trail

- EXTRACTED: 192 (98%)
- INFERRED: 4 (2%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*