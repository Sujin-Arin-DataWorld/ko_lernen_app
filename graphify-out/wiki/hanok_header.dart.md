# hanok_header.dart

> 39 nodes · cohesion 0.05

## Key Concepts

- **hanok_header.dart** (50 connections) — `lib/widgets/sori/hanok_header.dart`
- **../../services/audio_policy.dart** (6 connections)
- **package:video_player/video_player.dart** (5 connections)
- **VideoPlayerController** (5 connections)
- **VideoLeaseRequest** (4 connections) — `lib/widgets/sori/video_lease.dart`
- **SoriPosterLoop** (3 connections) — `lib/widgets/sori/hanok_header.dart`
- **_SoriPosterLoopState** (3 connections) — `lib/widgets/sori/hanok_header.dart`
- **video_lease.dart** (3 connections)
- **_Fallback** (2 connections) — `lib/widgets/sori/hanok_header.dart`
- **HanokHeader** (2 connections) — `lib/widgets/sori/hanok_header.dart`
- **_ambienceVolume** (1 connections) — `lib/widgets/sori/hanok_header.dart`
- **animate** (1 connections) — `lib/widgets/sori/hanok_header.dart`
- **_applyVolume** (1 connections) — `lib/widgets/sori/hanok_header.dart`
- **aspectRatio** (1 connections) — `lib/widgets/sori/hanok_header.dart`
- **asset** (1 connections) — `lib/widgets/sori/hanok_header.dart`
- **build** (1 connections) — `lib/widgets/sori/hanok_header.dart`
- **createState** (1 connections) — `lib/widgets/sori/hanok_header.dart`
- **didChangeDependencies** (1 connections) — `lib/widgets/sori/hanok_header.dart`
- **dispose** (1 connections) — `lib/widgets/sori/hanok_header.dart`
- **_eligibility** (1 connections) — `lib/widgets/sori/hanok_header.dart`
- **fallbackIcon** (1 connections) — `lib/widgets/sori/hanok_header.dart`
- **fallbackTint** (1 connections) — `lib/widgets/sori/hanok_header.dart`
- **fit** (1 connections) — `lib/widgets/sori/hanok_header.dart`
- **icon** (1 connections) — `lib/widgets/sori/hanok_header.dart`
- **initState** (1 connections) — `lib/widgets/sori/hanok_header.dart`
- *... and 14 more nodes in this community*

## Relationships

- [tiger_video.dart](tiger_video.dart.md) (5 shared connections)
- [intro_gate_screen.dart](intro_gate_screen.dart.md) (4 shared connections)
- [character_clip.dart](character_clip.dart.md) (4 shared connections)
- [video_lease.dart](video_lease.dart.md) (3 shared connections)
- [Color](Color.md) (2 shared connections)
- [_](_.md) (2 shared connections)
- [String?](String_.md) (2 shared connections)
- [StatelessWidget](StatelessWidget.md) (2 shared connections)
- [StatefulWidget](StatefulWidget.md) (2 shared connections)
- [managed_media_image.dart](managed_media_image.dart.md) (1 shared connections)
- [VideoLeaseEligibilityBinding](VideoLeaseEligibilityBinding.md) (1 shared connections)
- [dart:async](dart-async.md) (1 shared connections)

## Source Files

- `lib/widgets/sori/hanok_header.dart`
- `lib/widgets/sori/video_lease.dart`

## Audit Trail

- EXTRACTED: 73 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*