# check_personal_hanok_assets.py

> 24 nodes

## Key Concepts

- **check_personal_hanok_assets.py** (13 connections) — `tool/check_personal_hanok_assets.py`
- **PersonalHanokAssetCheckerTest** (8 connections) — `tool/test_check_personal_hanok_assets.py`
- **_check()** (5 connections) — `tool/check_personal_hanok_assets.py`
- **_compose_runtime_estate()** (5 connections) — `tool/check_personal_hanok_assets.py`
- **main()** (5 connections) — `tool/check_personal_hanok_assets.py`
- **_write_reference()** (5 connections) — `tool/check_personal_hanok_assets.py`
- **_check_reference()** (4 connections) — `tool/check_personal_hanok_assets.py`
- **_chroma_key_count()** (4 connections) — `tool/check_personal_hanok_assets.py`
- **Image** (4 connections)
- **_corners()** (3 connections) — `tool/check_personal_hanok_assets.py`
- **_coverage()** (3 connections) — `tool/check_personal_hanok_assets.py`
- **source_format()** (3 connections) — `tool/check_personal_hanok_assets.py`
- **test_check_personal_hanok_assets.py** (3 connections) — `tool/test_check_personal_hanok_assets.py`
- **_a1_runtime_leftovers()** (2 connections) — `tool/check_personal_hanok_assets.py`
- **.test_complete_runtime_a1_states_without_ledger_fail()** (2 connections) — `tool/test_check_personal_hanok_assets.py`
- **.test_qa_composite_is_outside_the_runtime_map()** (2 connections) — `tool/test_check_personal_hanok_assets.py`
- **Path** (2 connections)
- **.test_leftover_runtime_a1_files_fail_even_when_set_absent()** (1 connections) — `tool/test_check_personal_hanok_assets.py`
- **.test_partial_runtime_a1_states_fail_closed()** (1 connections) — `tool/test_check_personal_hanok_assets.py`
- **.test_repo_runtime_layers_and_qa_composite_pass()** (1 connections) — `tool/test_check_personal_hanok_assets.py`
- **.test_runtime_alias_of_the_qa_composite_fails()** (1 connections) — `tool/test_check_personal_hanok_assets.py`
- **.test_write_reference_refuses_the_runtime_map_folder()** (1 connections) — `tool/test_check_personal_hanok_assets.py`
- **Returns the same complete image the Flutter map paints at B2 100%.** (1 connections) — `tool/check_personal_hanok_assets.py`
- **Intentionally refreshes the QA-only reference from the approved layers.** (1 connections) — `tool/check_personal_hanok_assets.py`

## Relationships

- [load_provenance](load_provenance.md) (11 shared connections)
- [check_style_conformance.py](check_style_conformance.py.md) (1 shared connections)

## Source Files

- `tool/check_personal_hanok_assets.py`
- `tool/test_check_personal_hanok_assets.py`

## Audit Trail

- EXTRACTED: 41 (89%)
- INFERRED: 5 (11%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*