# dart:ui

> 73 nodes · cohesion 0.03

## Key Concepts

- **dart:ui** (58 connections)
- **scenarios_list_screen_ui_test.dart** (33 connections) — `test/scenarios_list_screen_ui_test.dart`
- **practice_hub_ui_test.dart** (30 connections) — `test/practice_hub_ui_test.dart`
- **hangul_stroke_order_test.dart** (14 connections) — `test/hangul_stroke_order_test.dart`
- **vocab_notebook_parser_test.dart** (10 connections) — `test/vocab_notebook_parser_test.dart`
- **stroke_matcher_test.dart** (9 connections) — `test/stroke_matcher_test.dart`
- **book_ocr_document_test.dart** (8 connections) — `test/book_ocr_document_test.dart`
- **snap_ocr_service_test.dart** (6 connections) — `test/snap_ocr_service_test.dart`
- **package:ko_lernen_app/services/snap_ocr_service.dart** (4 connections)
- **burst_asset_contract_test.dart** (4 connections) — `test/burst_asset_contract_test.dart`
- **package:ko_lernen_app/services/book_ocr_document.dart** (3 connections)
- **package:ko_lernen_app/services/stroke_matcher.dart** (2 connections)
- **Anfang** (1 connections)
- **HanokHeader** (1 connections)
- **package:ko_lernen_app/services/vocab_notebook_parser.dart** (1 connections)
- **package:ko_lernen_app/widgets/sori/badge.dart** (1 connections)
- **required List** (1 connections)
- **Schule** (1 connections)
- **documentFor** (1 connections) — `test/book_ocr_document_test.dart`
- **main** (1 connections) — `test/book_ocr_document_test.dart`
- **main** (1 connections) — `test/burst_asset_contract_test.dart`
- **canvas** (1 connections) — `test/hangul_stroke_order_test.dart`
- **circlePoints** (1 connections) — `test/hangul_stroke_order_test.dart`
- **hi** (1 connections) — `test/hangul_stroke_order_test.dart`
- **_isKnownPair** (1 connections) — `test/hangul_stroke_order_test.dart`
- *... and 48 more nodes in this community*

## Relationships

- [package:flutter_test/flutter_test.dart](package-flutter_test-flutter_test.dart.md) (14 shared connections)
- [bookshelf_custom_pack_uiux_test.dart](bookshelf_custom_pack_uiux_test.dart.md) (7 shared connections)
- [package:ko_lernen_app/l10n/generated/app_localizations.dart](package-ko_lernen_app-l10n-generated-app_localizations.dart.md) (7 shared connections)
- [_](_.md) (5 shared connections)
- [content_feedback_widget_test.dart](content_feedback_widget_test.dart.md) (4 shared connections)
- [package:shared_preferences/shared_preferences.dart](package-shared_preferences-shared_preferences.dart.md) (4 shared connections)
- [course_practice_screen_test.dart](course_practice_screen_test.dart.md) (3 shared connections)
- [package:ko_lernen_app/models/scenario.dart](package-ko_lernen_app-models-scenario.dart.md) (3 shared connections)
- [word_web_ui_test.dart](word_web_ui_test.dart.md) (3 shared connections)
- [standalone_games_uiux_test.dart](standalone_games_uiux_test.dart.md) (3 shared connections)
- [vocab_notebook_studio_screen_test.dart](vocab_notebook_studio_screen_test.dart.md) (3 shared connections)
- [book_analysis_security_test.dart](book_analysis_security_test.dart.md) (2 shared connections)

## Source Files

- `test/book_ocr_document_test.dart`
- `test/burst_asset_contract_test.dart`
- `test/hangul_stroke_order_test.dart`
- `test/practice_hub_ui_test.dart`
- `test/scenarios_list_screen_ui_test.dart`
- `test/snap_ocr_service_test.dart`
- `test/stroke_matcher_test.dart`
- `test/vocab_notebook_parser_test.dart`

## Audit Trail

- EXTRACTED: 167 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*