# tiger_video.dart

> 55 nodes · cohesion 0.04

## Key Concepts

- **tiger_video.dart** (72 connections) — `lib/widgets/sori/tiger_video.dart`
- **TigerGreetClip** (3 connections) — `lib/widgets/sori/tiger_video.dart`
- **_TigerGreetClipState** (3 connections) — `lib/widgets/sori/tiger_video.dart`
- **TigerStageVideo** (3 connections) — `lib/widgets/sori/tiger_video.dart`
- **_TigerStageVideoState** (3 connections) — `lib/widgets/sori/tiger_video.dart`
- **AudioPlayer?** (2 connections)
- **OneShotVideoLeaseCompletion** (2 connections) — `lib/widgets/sori/video_lease.dart`
- **_audio** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- **_audioStarted** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- **blendColor** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- **build** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- **_builtFor** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- **_completion** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- **createState** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- **didChangeDependencies** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- **didUpdateWidget** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- **dispose** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- **_eligibility** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- **_failed** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- **_fallback** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- **fallbackEmotion** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- **greetAsset** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- **greetFor** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- **_greetPlayedThisLaunch** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- **greetSfxFor** (1 connections) — `lib/widgets/sori/tiger_video.dart`
- *... and 30 more nodes in this community*

## Relationships

- [hanok_header.dart](hanok_header.dart.md) (5 shared connections)
- [StatefulWidget](StatefulWidget.md) (4 shared connections)
- [game_reward.dart](game_reward.dart.md) (2 shared connections)
- [../../l10n/generated/app_localizations.dart](-..-l10n-generated-app_localizations.dart.md) (2 shared connections)
- [character_clip.dart](character_clip.dart.md) (1 shared connections)
- [Color](Color.md) (1 shared connections)
- [mascot.dart](mascot.dart.md) (1 shared connections)
- [home_hero.dart](home_hero.dart.md) (1 shared connections)
- [VideoLeaseEligibilityBinding](VideoLeaseEligibilityBinding.md) (1 shared connections)
- [@visibleForTesting](@visibleForTesting.md) (1 shared connections)
- [static const String](static_const_String.md) (1 shared connections)
- [dart:async](dart-async.md) (1 shared connections)

## Source Files

- `lib/widgets/sori/tiger_video.dart`
- `lib/widgets/sori/video_lease.dart`

## Audit Trail

- EXTRACTED: 80 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*