# entry_onboarding_uiux_test.dart

> 67 nodes

## Key Concepts

- **entry_onboarding_uiux_test.dart** (110 connections) — `test/entry_onboarding_uiux_test.dart`
- **_MemoryJourneyRepository? repository,
  LegacyOnboardingSnapshot** (1 connections)
- **PageView** (1 connections)
- **bounds** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **box** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **browse** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **candidates** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **_cardWithText** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **_centerInScrollable** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **clear** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **companion** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **_coordinator** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **data** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **disableAnimations** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **_disposeEntry** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **ensureVisible** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **_EntryFixture** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **_expectAction** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **_expectFullyVisible** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **_expectPointerOwned** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **_expectReachableInSafeArea** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **failNextLoad** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **failNextSave** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **finder** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- **_FinderForLocale** (1 connections) — `test/entry_onboarding_uiux_test.dart`
- *... and 42 more nodes in this community*

## Relationships

- [package:ko_lernen_app/l10n/generated/app_localizations.dart](package-ko_lernen_app-l10n-generated-app_localizations.dart.md) (8 shared connections)
- [package:flutter_test/flutter_test.dart](package-flutter_test-flutter_test.dart.md) (5 shared connections)
- [app_flows_test.dart](app_flows_test.dart.md) (5 shared connections)
- [course_reassessment_screen_test.dart](course_reassessment_screen_test.dart.md) (3 shared connections)
- [app_shell_onboarding_analytics_retry_test.dart](app_shell_onboarding_analytics_retry_test.dart.md) (3 shared connections)
- [content_feedback_widget_test.dart](content_feedback_widget_test.dart.md) (3 shared connections)
- [onboarding_journey_state.dart](onboarding_journey_state.dart.md) (2 shared connections)
- [custom_pack_games_uiux_test.dart](custom_pack_games_uiux_test.dart.md) (1 shared connections)
- [cloud_sync_test.dart](cloud_sync_test.dart.md) (1 shared connections)
- [first_run_coordinator_test.dart](first_run_coordinator_test.dart.md) (1 shared connections)
- [learning_semantic_contract.dart](learning_semantic_contract.dart.md) (1 shared connections)
- [return](return.md) (1 shared connections)

## Source Files

- `test/entry_onboarding_uiux_test.dart`

## Audit Trail

- EXTRACTED: 110 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*