# crop_recovery_service.dart

> 25 nodes

## Key Concepts

- **crop_recovery_service.dart** (31 connections) — `lib/services/crop_recovery_service.dart`
- **CropRecoveryGateway** (3 connections) — `lib/services/crop_recovery_service.dart`
- **ImageCropperRecoveryGateway** (2 connections) — `lib/services/crop_recovery_service.dart`
- **package:image_cropper/image_cropper.dart** (2 connections)
- **_CropGateway** (2 connections) — `test/crop_recovery_test.dart`
- **ImageCropper** (1 connections)
- **acceptBookCrop** (1 connections) — `lib/services/crop_recovery_service.dart`
- **BookCropAcceptance** (1 connections) — `lib/services/crop_recovery_service.dart`
- **BookCropSession** (1 connections) — `lib/services/crop_recovery_service.dart`
- **_cropMarker** (1 connections) — `lib/services/crop_recovery_service.dart`
- **_cropper** (1 connections) — `lib/services/crop_recovery_service.dart`
- **CropRecoveryService** (1 connections) — `lib/services/crop_recovery_service.dart`
- **deleteTrustedTemporary** (1 connections) — `lib/services/crop_recovery_service.dart`
- **displacedLease** (1 connections) — `lib/services/crop_recovery_service.dart`
- **isAndroid** (1 connections) — `lib/services/crop_recovery_service.dart`
- **lease** (1 connections) — `lib/services/crop_recovery_service.dart`
- **previous** (1 connections) — `lib/services/crop_recovery_service.dart`
- **previousRecord** (1 connections) — `lib/services/crop_recovery_service.dart`
- **_recover** (1 connections) — `lib/services/crop_recovery_service.dart`
- **recoverAtStartup** (1 connections) — `lib/services/crop_recovery_service.dart`
- **RecoveredBookPersistence** (1 connections) — `lib/services/crop_recovery_service.dart`
- **recoverImagePath** (1 connections) — `lib/services/crop_recovery_service.dart`
- **resetForTesting** (1 connections) — `lib/services/crop_recovery_service.dart`
- **_startupOperation** (1 connections) — `lib/services/crop_recovery_service.dart`
- **store** (1 connections) — `lib/services/crop_recovery_service.dart`

## Relationships

- [media_workflow.dart](media_workflow.dart.md) (1 shared connections)
- [first_link_backfill_journal.dart](first_link_backfill_journal.dart.md) (1 shared connections)
- [String?](String_.md) (1 shared connections)
- [typedef](typedef.md) (1 shared connections)
- [picker_recovery_service.dart](picker_recovery_service.dart.md) (1 shared connections)
- [package:flutter_test/flutter_test.dart](package-flutter_test-flutter_test.dart.md) (1 shared connections)
- [dart:io](dart-io.md) (1 shared connections)
- [storage_service.dart](storage_service.dart.md) (1 shared connections)
- [book_capture_screen.dart](book_capture_screen.dart.md) (1 shared connections)
- [dart:ui](dart-ui.md) (1 shared connections)

## Source Files

- `lib/services/crop_recovery_service.dart`
- `test/crop_recovery_test.dart`

## Audit Trail

- EXTRACTED: 35 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*