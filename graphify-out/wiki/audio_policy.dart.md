# audio_policy.dart

> 34 nodes

## Key Concepts

- **audio_policy.dart** (42 connections) — `lib/services/audio_policy.dart`
- **buildAndroidContext** (2 connections) — `lib/services/audio_policy.dart`
- **gainFor** (2 connections) — `lib/services/audio_policy.dart`
- **SoundChannel** (2 connections) — `lib/services/audio_policy.dart`
- **_ambienceGain** (1 connections) — `lib/services/audio_policy.dart`
- **applyPlatformAudioContext** (1 connections) — `lib/services/audio_policy.dart`
- **_clamp01** (1 connections) — `lib/services/audio_policy.dart`
- **_defaultOn** (1 connections) — `lib/services/audio_policy.dart`
- **defaultOnOf** (1 connections) — `lib/services/audio_policy.dart`
- **_defaultVol** (1 connections) — `lib/services/audio_policy.dart`
- **defaultVolumeOf** (1 connections) — `lib/services/audio_policy.dart`
- **dispose** (1 connections) — `lib/services/audio_policy.dart`
- **_duckActive** (1 connections) — `lib/services/audio_policy.dart`
- **_duckFactor** (1 connections) — `lib/services/audio_policy.dart`
- **duckOnSpeech** (1 connections) — `lib/services/audio_policy.dart`
- **_duckRestore** (1 connections) — `lib/services/audio_policy.dart`
- **idOf** (1 connections) — `lib/services/audio_policy.dart`
- **instance** (1 connections) — `lib/services/audio_policy.dart`
- **isOn** (1 connections) — `lib/services/audio_policy.dart`
- **masterOn** (1 connections) — `lib/services/audio_policy.dart`
- **masterVolume** (1 connections) — `lib/services/audio_policy.dart`
- **noteSpeechEnded** (1 connections) — `lib/services/audio_policy.dart`
- **noteSpeechStarted** (1 connections) — `lib/services/audio_policy.dart`
- **respectSilentMode** (1 connections) — `lib/services/audio_policy.dart`
- **restoreDuckNow** (1 connections) — `lib/services/audio_policy.dart`
- *... and 9 more nodes in this community*

## Relationships

- [@visibleForTesting](@visibleForTesting.md) (2 shared connections)
- [bool get](bool_get.md) (1 shared connections)
- [double get](double_get.md) (1 shared connections)
- [hangul_screen.dart](hangul_screen.dart.md) (1 shared connections)
- [data_migration_service.dart](data_migration_service.dart.md) (1 shared connections)
- [listening_playback_controller.dart](listening_playback_controller.dart.md) (1 shared connections)
- [dart:async](dart-async.md) (1 shared connections)
- [package:flutter_test/flutter_test.dart](package-flutter_test-flutter_test.dart.md) (1 shared connections)
- [package:flutter/foundation.dart](package-flutter-foundation.dart.md) (1 shared connections)
- [storage_service.dart](storage_service.dart.md) (1 shared connections)
- [_](_.md) (1 shared connections)

## Source Files

- `lib/services/audio_policy.dart`

## Audit Trail

- EXTRACTED: 45 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*