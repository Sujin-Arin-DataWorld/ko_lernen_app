# ../models/course_mastery.dart

> 20 nodes · cohesion 0.12

## Key Concepts

- **../models/course_mastery.dart** (19 connections)
- **_** (18 connections) — `lib/services/hanok_build_narrative_service.dart`
- **_** (15 connections) — `lib/services/hanok_structure_projection_service.dart`
- **course_mastery_service.dart** (5 connections)
- **scenario_loader.dart** (3 connections)
- **../models/hanok_competence.dart** (2 connections)
- **hanok_stage_service.dart** (1 connections)
- **HanokBuildNarrativeService** (1 connections) — `lib/services/hanok_build_narrative_service.dart`
- **HanokNarrativeCatalogLoader** (1 connections) — `lib/services/hanok_build_narrative_service.dart`
- **HanokNarrativeScenarioLoader** (1 connections) — `lib/services/hanok_build_narrative_service.dart`
- **HanokNarrativeSnapshotReader** (1 connections) — `lib/services/hanok_build_narrative_service.dart`
- **loadForProjection** (1 connections) — `lib/services/hanok_build_narrative_service.dart`
- **loadReceipt** (1 connections) — `lib/services/hanok_build_narrative_service.dart`
- **_readStoredSnapshot** (1 connections) — `lib/services/hanok_build_narrative_service.dart`
- **HanokStructureCatalogLoader** (1 connections) — `lib/services/hanok_structure_projection_service.dart`
- **HanokStructureProjectionService** (1 connections) — `lib/services/hanok_structure_projection_service.dart`
- **HanokStructureSnapshotReader** (1 connections) — `lib/services/hanok_structure_projection_service.dart`
- **loadCurrent** (1 connections) — `lib/services/hanok_structure_projection_service.dart`
- **loadForRatios** (1 connections) — `lib/services/hanok_structure_projection_service.dart`
- **_readStoredSnapshot** (1 connections) — `lib/services/hanok_structure_projection_service.dart`

## Relationships

- [_](_.md) (4 shared connections)
- [account_ui_operations.dart](account_ui_operations.dart.md) (2 shared connections)
- [../../models/curriculum.dart](-..-models-curriculum.dart.md) (2 shared connections)
- [course_progress_service.dart](course_progress_service.dart.md) (2 shared connections)
- [typedef](typedef.md) (2 shared connections)
- [gye_weekly_promise_navigation.dart](gye_weekly_promise_navigation.dart.md) (2 shared connections)
- [../../models/personal_hanok.dart](-..-models-personal_hanok.dart.md) (2 shared connections)
- [scenario_player_screen.dart](scenario_player_screen.dart.md) (2 shared connections)
- [hanok_experience_projector.dart](hanok_experience_projector.dart.md) (2 shared connections)
- [hanok_world_screen.dart](hanok_world_screen.dart.md) (1 shared connections)
- [kkeunmari_engine.dart](kkeunmari_engine.dart.md) (1 shared connections)
- [course_mission_screen.dart](course_mission_screen.dart.md) (1 shared connections)

## Source Files

- `lib/services/hanok_build_narrative_service.dart`
- `lib/services/hanok_structure_projection_service.dart`

## Audit Trail

- EXTRACTED: 54 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*
