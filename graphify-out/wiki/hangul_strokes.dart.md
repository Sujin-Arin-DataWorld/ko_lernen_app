# hangul_strokes.dart

> 11 nodes

## Key Concepts

- **hangul_strokes.dart** (13 connections) — `lib/data/hangul_strokes.dart`
- **Size** (13 connections)
- **Offset?** (8 connections)
- **Stroke** (3 connections) — `lib/data/hangul_strokes.dart`
- **CircleStroke** (2 connections) — `lib/data/hangul_strokes.dart`
- **LineStroke** (2 connections) — `lib/data/hangul_strokes.dart`
- **center** (1 connections) — `lib/data/hangul_strokes.dart`
- **hangulStrokes** (1 connections) — `lib/data/hangul_strokes.dart`
- **points** (1 connections) — `lib/data/hangul_strokes.dart`
- **radius** (1 connections) — `lib/data/hangul_strokes.dart`
- **strokeCanvas** (1 connections) — `lib/data/hangul_strokes.dart`

## Relationships

- [ildu_world_screen.dart](ildu_world_screen.dart.md) (2 shared connections)
- [hanok_world_screen.dart](hanok_world_screen.dart.md) (2 shared connections)
- [List](List.md) (1 shared connections)
- [Map](Map.md) (1 shared connections)
- [package:flutter/material.dart](package-flutter-material.dart.md) (1 shared connections)
- [shelf_case.dart](shelf_case.dart.md) (1 shared connections)
- [swipe_card.dart](swipe_card.dart.md) (1 shared connections)
- [celebration.dart](celebration.dart.md) (1 shared connections)
- [dancheong_burst.dart](dancheong_burst.dart.md) (1 shared connections)
- [dart:math](dart-math.md) (1 shared connections)
- [custom_pack_games_uiux_test.dart](custom_pack_games_uiux_test.dart.md) (1 shared connections)
- [ildu_turntable_catalog.dart](ildu_turntable_catalog.dart.md) (1 shared connections)

## Source Files

- `lib/data/hangul_strokes.dart`

## Audit Trail

- EXTRACTED: 34 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*