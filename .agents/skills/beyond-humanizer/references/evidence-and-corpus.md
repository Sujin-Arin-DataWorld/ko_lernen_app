...
다음 판단은 현재 용례 근거 없이 `MODEL_QA_PASS`로 닫지 않는다.

- 신조어·줄임말·온라인 집단어와 세대 표지
- 공개 댓글과 사적 채팅의 관계 경계
- 화자 직감이 갈리는 연어·종결형·호칭
- “2026년에 자연스럽다”, “요즘은 이렇게 말한다” 같은 최신성 주장

이 경우 질문, corpus 유형, 목표 장르·관계, 분포, 반례, 접근·라이선스 상태를 기록한다. 국립국어원의 `온라인 게시 자료 말뭉치 2025`, `일상 대화 음성 말뭉치 2025`, `일상 대화 말뭉치 2025`는 현대 한국어 후보를 확인하는 우선 자료다. 문장을 제품 데이터로 복사하지 않는다.

안정적인 일상 표현은 Scene Lock, 언어 렌즈, target-only 감사가 모두 통과하면 실시간 corpus 검색 없이 인간 검수 단계로 보낼 수 있다. corpus 비필수는 인간 승인을 뜻하지 않는다. 필수 근거를 접근할 수 없으면 상태는 `EVIDENCE_REQUIRED`다.
...
모델 self-audit은 독립 판정이 아니다. 같은 모델이 만든 번역을 스스로 통과시킨 결과는 failure discovery 자료로 쓰되, GREEN 근거는 별도 fresh-context 검토와 가능한 인간 gate를 요구한다. 자동 통과를 “언어학자 검증 완료”로 표현하지 않고 상태를 `MODEL_QA_PASS`로 제한한다.
