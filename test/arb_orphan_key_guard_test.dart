...
/// 로드+토큰화+비교 전체 약 624ms. 2026-09-03 W-F/W-G 착지 + W-J J1(파일럿
/// 삭제) 후 같은 스코프·방법으로 재측정한 값은 **317개**(W-E 가 이미 지운
/// "활동 준비됨" 키는 삭제 유지 승인 — §COPY-5 D19) — 앞으로 소수 차이가 나면 이
/// 파일이 그 자리에서 직접 재측정한 값이 우선한다. "하지 말 것" 원칙
/// (래칫은 내려가기만 한다) — 새 고아 키를 늘리는 PR은 이 테스트를 반드시
/// 빨갛게 만든다. 고아를 줄였다면 ceiling 도 그 값으로 내릴 것(절대 올리지
/// 말 것).
///
...
void main() {
  test('ARB 고아 키 수가 상한(래칫)을 넘지 않는다', () {
    ...
    // §COPY-5(J15): 2026-09-03 W-F/W-G 착지(+ W-J J1 파일럿 삭제) 후
    // 같은 스코프·방법으로 재실측 — 317개(2026-09-01 값 319에서 하향).
    const ceiling = 317;
    expect(
      orphaned.length,
