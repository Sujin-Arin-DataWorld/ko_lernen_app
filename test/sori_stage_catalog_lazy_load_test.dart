...
void main() {
  testWidgets('active:false 탭은 loadSnapshot 을 호출하지 않는다 (P4-2 lazy load)', (
    tester,
  ) async {
    var calls = 0;
  ...
  testWidgets('탭이 나중에 active 가 되면 그때 loadSnapshot 을 1회 호출한다', (tester) async {
    var calls = 0;
    var active = false;
    await tester.pumpWidget(
      StatefulBuilder(
  ...
  testWidgets('비활성화 후 재활성화하면 재활성화마다 정확히 1회씩 로드한다 '
      '(재활성화 계약 — 3회 이상 금지)', (tester) async {
    var calls = 0;
    var active = false;
    await tester.pumpWidget(
      StatefulBuilder(
