...
void main() {
  ...
  testWidgets('작은 delta가 누적되는 느린 드래그도 축을 잠그고 판정한다', (tester) async {
    var rights = 0;
    var downs = 0;
    await tester.pumpWidget(
      host(onRight: () => rights++, onLeft: () {}, onDown: () => downs++),
    );
    ...
    for (var step = 0; step < 28; step++) {
      // No individual update reaches the 12dp axis-lock threshold. The raw
      // movement must therefore be accumulated across updates.
      await gesture.moveBy(const Offset(8, 1));
  ...
  testWidgets('null vertical callback disables only that direction and badge', (
    tester,
  ) async {
    var downs = 0;
    await tester.pumpWidget(host(onDown: () => downs++));
    ...
    await tester.drag(card, const Offset(0, 160));
    await tester.pumpAndSettle();
    expect(downs, 1);
  });

  testWidgets('normal-motion cancel springs back and remains usable', (
    tester,
  ) async {
    var rights = 0;
    await tester.pumpWidget(
      host(onLeft: () {}, onRight: () => rights++, disableAnimations: false),
    );
  ...
  testWidgets('normal-motion exit ignores a rapid second gesture', (
    tester,
  ) async {
    var rights = 0;
    await tester.pumpWidget(
