...
class CurrentReviewIndexTest(unittest.TestCase):
    def test_index_is_bound_to_the_locked_120_candidate_receipts(self) -> None:
        ...
        # The original editorial approval predates PR286's A2 -> B1 relevel.
        # Rebinding a review index must not rewrite or inherit that approval.
        self.assertEqual(receipt_hashes, {
            "63dc5ea8743ec7f95fed04c5efb2a32dd2445c310444e87237414c0ced980060"
        })
        self.assertNotIn(index["candidateSetSha256"], receipt_hashes)
        self.assertEqual(index["reviewStatus"], "human_gate")
    ...
    def test_checked_in_index_matches_the_current_locked_candidates(self) -> None:
        ...
        self.assertEqual(
            review.verify_current_index(checked_in, root=ROOT),
            {
                ...
                "candidateCount": 120,
                "candidateSetSha256": (
                    "80d039cc9abaa03dff526f73ac86201c"
                    "76888b27bfe637add38f0989279e68d0"
                ),
            },
        )
...
class OverlayValidationTest(unittest.TestCase):
    ...
    def test_taxi_rejects_indirect_deductions_in_all_review_languages(self) -> None:
        cases = (
            ("ko", "네. 여기 세워 주세요.", "네. 돈은 앱에서 알아서 빠져나가요. 여기 세워 주세요."),
            ("de", "Ja. Bitte halten Sie hier.", "Ja. Das Geld geht automatisch vom Konto ab. Bitte halten Sie hier."),
            ("en", "Yes. Please stop here.", "Yes. The amount is taken automatically from your account. Please stop here."),
        )
        for language, before, after in cases:
            for status in ("draft_change", "human_gate"):
                with self.subTest(language=language, status=status):
                    overlay = self._overlay(before=before, after=after)
                    change = overlay["scenarios"][0]["changes"][0]
                    change["path"] = f"/dialog/1/{language}"
...
class CompleteScenarioReviewTest(unittest.TestCase):
    def test_review_includes_complete_current_scenario_for_human_comparison(self) -> None:
        candidate = ROOT / "tools/content_factory/review/canonical_120_v1/candidates/a1/taxi_kakao.json"
        before = candidate.read_bytes()
        expected = json.loads(before)["scenario"]
        rendered = review.render_scenario_review("taxi_kakao", root=ROOT)
        block = re.search(r"```json\n(.*?)\n```", rendered, re.DOTALL)
        self.assertIsNotNone(block, "the full current scenario must accompany the dialogue")
        self.assertEqual(json.loads(block.group(1)), expected)
        for field in ("title", "intro", "vocab", "grammarBlock", "dialog", "quests", "culturalNote"):
