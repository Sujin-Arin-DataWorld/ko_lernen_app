#!/usr/bin/env python3
"""Emit review-only Batch 15 scenario drafts: C1 기능 확장 7칸.
...
DRAFT_PATH = Path("tools/content_factory/drafts/c1_batch15_scenarios_c1.json")
MANIFEST_PATH = Path("tools/content_factory/drafts/batch_15_manifest.json")
REVIEW_PATH = Path("tools/content_factory/review/c1_batch15_scenarios.csv")
...
CATEGORY_ORDER = ("conflict_interest", "policy", "clinical", "critique", "mediation", "facework", "attribution")
...
def build(root: Path = ROOT) -> dict[str, int]:
    ...
    draft = {
        "version": 1,
        "_comment": "Review-only Batch 15 scenarios. C1 expansion shelves, four per shelf.",
        "scenarios": records,
    }
    manifest = {
        "version": 1,
        ...
        "provenance": {
            "scope": "Original A1 episodes for the C1 expansion shelves.",
            "rights": "original",
