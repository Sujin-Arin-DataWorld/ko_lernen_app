...
| 파일 | 열 | 행수 | 정렬 |
|---|---|---|---|
| `nikl_kiiq_2017_vocab.csv` | `grade,headword,homograph,pos,guide,band` | 11,101(구분자 분리 후) | `(grade,headword,homograph)` |
...
- **kiiq 어휘 (구분자 분리 후, CSV에 실제로 쓰는 행수)**: 1급 810 · 2급 1,134 · 3급 1,713 · 4급 2,285 · 5급 2,457 · 6급 2,702 (합계 11,101)
...
2. 표제어가 `/`, `∙`(U+2219 bullet operator), `·`(U+00B7 가운뎃점),
   `•`(U+2022 bullet), `‧`(U+2027 hyphenation point) 중 무엇으로든 묶여 있으면(`오늘02/오늘01`,
   `마흔02∙마흔`) 행을 분리한다 — 다섯 구분자 모두 동등하게 취급한다. 2017
   ...
   U+2027은 `천만02‧천만01/천만03`, `셋째02‧셋째01/셋째03`,
   `첫째02‧첫째01/첫째03`, `소극적01‧소극적02` 네 원본 행에 존재한다.
   같은 위치의 품사를 보존해 분리하며 길잡이말에는 이 규칙을 적용하지 않는다.
