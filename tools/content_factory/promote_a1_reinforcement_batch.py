...
{rows.csv,cloze.json,satz.json} -- this script only:
  ...
  2. adds any missing clozeTopicUnitMap routes;
...
tool/generate_tts.py --missing-from-storage / --verify-storage), level
reports (tool/audit_content_levels.py, tool/audit_vocab_levels.py, tool/
build_level_bible_tables.py -- restore docs/data/level_bible/F9_exceptions.
md's hand-curated rows afterward), the curriculum/phase CI gate (tool/
audit_curriculum_matrix.py --check, tool/audit_learning_phases.py --check
--quiet, tool/build_phase_tasks.py --check, python -m tool.
audit_phase_context_evidence --check, tool/build_learning_phase_catalog.py
--check, tool/build_curriculum_backlog.py --check -- run each WITHOUT
--check first wherever stale), vocab_pack_map.md / vocab_level_report.md /
...
def main(batch: int, apply: bool) -> None:
    ...
    draft_satz = rj(draft_satz_path)["items"]
    ...
    # -- courseUnitId / canDoSegmentId lookup --
    ...
    print(f"batch {batch}: planned {len(draft_rows)} vocab, {len(draft_cloze)} cloze, {len(draft_satz)} satz")
    ...
    wj(draft_cloze_path, {"items": draft_cloze})
    wj(draft_satz_path, {"items": draft_satz})
