#!/usr/bin/env python3
"""Batch 13(A1 확장 3칸) 초안의 계약·초안↔live 일치·review projection 회귀.
...
import build_batch_13_scenarios as builder
from data.batch_13_scene_scripts import SCENES

ROOT = SCRIPT_DIR.parents[1]
CATEGORIES = ("numbers", "phone", "wayfinding")
LEVELS = ("a1",)
...
class ShelfSlugContractTest(unittest.TestCase):
    def test_categories_are_the_declared_a1_expansion_slugs(self):
        # 카테고리 철자가 shelf_assignment 의 확장 slug 와 어긋나면 칸이 조용히 빈다.
        from shelf_assignment import EXPANSION_SLUGS

        self.assertEqual(sorted(CATEGORIES), sorted(EXPANSION_SLUGS["a1"]))
...
class BuildOutputTest(unittest.TestCase):
    def setUp(self):
        ...
        self.draft = json.loads(
            (ROOT / "tools/content_factory/drafts/c1_batch13_scenarios_a1.json").read_text(encoding="utf-8"))
        self.manifest = json.loads(
            (ROOT / "tools/content_factory/drafts/batch_13_manifest.json").read_text(encoding="utf-8"))
        with (ROOT / "tools/content_factory/review/c1_batch13_scenarios.csv").open(
    ...
    def test_manifest_stays_review_only(self):
        ...
        self.assertEqual(self.manifest["batch"], "13")
