#!/usr/bin/env python3
"""Batch 15(C1 확장 7칸) 초안의 계약·초안↔live 일치·review projection 회귀.
...
import build_batch_15_scenarios as builder
from data.batch_15_scene_scripts import SCENES

ROOT = SCRIPT_DIR.parents[1]
CATEGORIES = ("conflict_interest", "policy", "clinical", "critique",
              "mediation", "facework", "attribution")
LEVELS = ("c1",)
...
class ShelfSlugContractTest(unittest.TestCase):
    def test_categories_are_the_declared_expansion_slugs(self):
        ...
        # C1 은 확장 3칸(methodology·facework·attribution)뿐 아니라 기능 5칸
        # (conflict_interest·policy·clinical·critique·mediation)도 비어 있었다.
        # 그래서 이 배치의 카테고리는 확장 slug 의 부분집합이 아니라 **그 레벨의
        # 선언된 slug 전체**의 부분집합이어야 한다.
        for level in LEVELS:
            authored = {s["category"] for s in SCENES if s["level"] == level}
            from shelf_assignment import SHELF_SLUGS

            declared = set(SHELF_SLUGS[level])
            self.assertTrue(
                authored <= declared,
                f"{level} 의 집필 카테고리가 선언된 slug 밖이다: "
                f"{sorted(authored - declared)}",
            )
...
class BuildOutputTest(unittest.TestCase):
    def setUp(self):
        ...
        self.draft = json.loads(
            (ROOT / "tools/content_factory/drafts/c1_batch15_scenarios_c1.json").read_text(encoding="utf-8"))
        self.manifest = json.loads(
            (ROOT / "tools/content_factory/drafts/batch_15_manifest.json").read_text(encoding="utf-8"))
        with (ROOT / "tools/content_factory/review/c1_batch15_scenarios.csv").open(
    ...
    def test_manifest_stays_review_only(self):
        ...
        self.assertEqual(self.manifest["batch"], "15")
