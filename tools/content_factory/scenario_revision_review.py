...
_TAXI_PAYMENT_CONCEPT_PATTERNS = (
    # Conservative lexical screening, never a substitute for human fact review.
    re.compile(r"(?:결제|청구|자동\s*납부|카드\s*승인|출금|차감|인출|금액|요금|대금|돈(?:[은이을도만]|\b))"),
    re.compile(
        r"\b(?:pay(?:ment|ments|ing|ed|s)?|paid|fare(?:s)?|"
        r"charg(?:e|ed|es|ing)|bill(?:ed|ing|s)?|debit(?:ed|ing|s)?|"
        r"money|cash|funds|deduct\w*|withdraw\w*)\b",
        re.IGNORECASE,
    ),
    re.compile(r"\bamount\b[^.!?\n]{0,100}\b(?:taken|removed)\b", re.IGNORECASE),
    re.compile(
        r"\b(?:zahlung\w*|zahl\w*|bezahl\w*|abbuch\w*|belast\w*|"
        r"abrechn\w*|fahrpreis\w*|geld\w*|betrag\w*|konto\w*)\b",
        re.IGNORECASE,
    ),
)
...
def render_scenario_review(scenario_id: str, root: Path = ROOT) -> str:
    ...
    lines.extend(
        [
            ...
            "## 현재 장면 전체",
            "",
            "제목·소개·어휘·문법·퀘스트·문화 설명과 변경하지 않을 구조를 함께 검토합니다.",
            "",
            "```json",
