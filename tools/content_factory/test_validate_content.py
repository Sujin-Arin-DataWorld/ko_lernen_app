...
class ContentValidatorTest(unittest.TestCase):
    ...
    def test_audit_graph_counts_must_match_curriculum(self) -> None:
        ...
        messages = self._messages(validator)
        self.assertTrue(any("graph.courseUnits" in message for message in messages))
        self.assertTrue(any("graph.c2CourseUnits" in message for message in messages))
